/*
 *  kuzpkg-conf.c - parse kuzpkg configuration files
 *
...
const char *myname = "kuzpkg-conf", *myver = "1.0.0";

klpm_list_t *directives = NULL;
char sep = '\n', *repo_name = NULL;
...
static void usage(int ret)
{
	...
	fputs(_("kuzpkg-conf - query kuzpkg's configuration file\n"), stream);
	fputs(_("usage:  kuzpkg-conf [options] [<directive>...]\n"), stream);
	fputs(_("        kuzpkg-conf (--repo-list|--help|--version)\n"), stream);
	fputs(_("options:\n"), stream);
...
static void show_list_str(const char *directive, klpm_list_t *list)
{
	klpm_list_t *i;
...
static void show_siglevel(const char *directive, klpm_siglevel_t level, int pkgonly)
{
	...
	if(level & KUZPKG_SIG_PACKAGE) {
		if(level & KUZPKG_SIG_PACKAGE_OPTIONAL) {
			show_str(directive, "PackageOptional");
		} else {
			show_str(directive, "PackageRequired");
		}

		if(level & KUZPKG_SIG_PACKAGE_UNKNOWN_OK) {
	...
	if(level & KUZPKG_SIG_DATABASE) {
		if(level & KUZPKG_SIG_DATABASE_OPTIONAL) {
			show_str(directive, "DatabaseOptional");
		} else {
			show_str(directive, "DatabaseRequired");
		}

		if(level & KUZPKG_SIG_DATABASE_UNKNOWN_OK) {
...
{
	if(usage == KUZPKG_DB_USAGE_ALL) {
		show_str(directive, "All");
	} else {
		if(usage & KUZPKG_DB_USAGE_SYNC) {
			show_str(directive, "Sync");
		}
		if(usage & KUZPKG_DB_USAGE_SEARCH) {
			show_str(directive, "Search");
		}
		if(usage & KUZPKG_DB_USAGE_INSTALL) {
			show_str(directive, "Install");
		}
		if(usage & KUZPKG_DB_USAGE_UPGRADE) {
...
int main(int argc, char **argv)
{
	...
	for(; optind < argc; optind++) {
		directives = klpm_list_add(directives, argv[optind]);
	}

	if(klpm_list_count(directives) != 1) {
		verbose = 1;
	}
