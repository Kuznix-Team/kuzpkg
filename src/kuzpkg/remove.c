...
static int remove_target(const char *target)
{
	...
	if((pkg = klpm_db_get_pkg(db_local, target)) != NULL) {
		if(klpm_remove_pkg(config->handle, pkg) == -1) {
			klpm_errno_t err = klpm_errno(config->handle);
			pm_printf(KUZPKG_LOG_ERROR, "'%s': %s\n", target, klpm_strerror(err));
			return -1;
		}
		config->explicit_removes = klpm_list_add(config->explicit_removes, pkg);
		return 0;
	}

		/* fallback to group */
	klpm_group_t *grp = klpm_db_get_group(db_local, target);
	if(grp == NULL) {
...
int kuzpkg_remove(klpm_list_t *targets)
{
	...
	for(i = klpm_trans_get_remove(config->handle); i; i = klpm_list_next(i)) {
		...
		if(klpm_list_find(config->holdpkg, klpm_pkg_get_name(pkg), fnmatch_cmp)) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s is designated as a HoldPkg.\n"),
	...
	klpm_list_t *pkglist = klpm_trans_get_remove(config->handle);
	if(pkglist == NULL) {
