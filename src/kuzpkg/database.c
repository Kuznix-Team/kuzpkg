 ...
 */
static int change_install_reason(klpm_list_t *targets)
{
	klpm_list_t *i;
	klpm_db_t *db_local;
	int ret = 0;
	...
	if(config->flags & KUZPKG_TRANS_FLAG_ALLDEPS) { /* --asdeps */
		reason = KUZPKG_PKG_REASON_DEPEND;
	} else if(config->flags & KUZPKG_TRANS_FLAG_ALLEXPLICIT) { /* --asexplicit */
		reason = KUZPKG_PKG_REASON_EXPLICIT;
	} else {
		pm_printf(KUZPKG_LOG_ERROR, _("no install reason specified (use -h for help)\n"));
		return 1;
	}
	...
	for(i = targets; i; i = klpm_list_next(i)) {
		char *pkgname = i->data;
		klpm_pkg_t *pkg = klpm_db_get_pkg(db_local, pkgname);
		if(!pkg || klpm_pkg_set_reason(pkg, reason)) {
			pm_printf(KUZPKG_LOG_ERROR, _("could not set install reason for package %s (%s)\n"),
							pkgname, klpm_strerror(klpm_errno(config->handle)));
			ret = 1;
		} else {
			if(!config->quiet) {
				if(reason == KUZPKG_PKG_REASON_DEPEND) {
...
static int check_db_missing_deps(klpm_list_t *pkglist)
{
	klpm_list_t *data, *i;
	int ret = 0;
	/* check dependencies */
	data = klpm_checkdeps(config->handle, NULL, NULL, pkglist, 0);
	for(i = data; i; i = klpm_list_next(i)) {
		klpm_depmissing_t *miss = i->data;
		char *depstring = klpm_dep_compute_string(miss->depend);
		pm_printf(KUZPKG_LOG_ERROR, "missing '%s' dependency for '%s'\n",
				depstring, miss->target);
		free(depstring);
		ret = 1;
	}
	klpm_list_free_inner(data, (klpm_list_fn_free)klpm_depmissing_free);
	klpm_list_free(data);
	return ret;
}
...
static int check_db_local_files(void)
{
	...
	if(!(dbdir = opendir(path))) {
		pm_printf(KUZPKG_LOG_ERROR, "could not open local database directory %s: %s\n",
				path, strerror(errno));
		return 1;
	}
...
static int check_db_local_package_conflicts(klpm_list_t *pkglist)
{
	klpm_list_t *data, *i;
	int ret = 0;
	/* check conflicts */
	data = klpm_checkconflicts(config->handle, pkglist);
	for(i = data; i; i = i->next) {
		klpm_conflict_t *conflict = i->data;
		pm_printf(KUZPKG_LOG_ERROR, "'%s' conflicts with '%s'\n",
				klpm_pkg_get_name(conflict->package1), klpm_pkg_get_name(conflict->package2));
		ret = 1;
	}
	klpm_list_free_inner(data, (klpm_list_fn_free)klpm_conflict_free);
	klpm_list_free(data);
	return ret;
}
...
static int check_db_local_filelist_conflicts(klpm_list_t *pkglist)
{
	klpm_list_t *i;
	int ret = 0;
	...
	for(i = pkglist; i; i = i->next) {
		...
		for(j = 0; j < filelist->count; j++) {
			klpm_file_t *file = filelist->files + j;
	...
	for(j = 0; j < offset; j++) {
		...
		if(prev_fileitem && fileitem_cmp(prev_fileitem, fileitem) == 0) {
			pm_printf(KUZPKG_LOG_ERROR, "file owned by '%s' and '%s': '%s'\n",
					klpm_pkg_get_name(prev_fileitem->pkg),
					klpm_pkg_get_name(fileitem->pkg),
					fileitem->file->name);
			ret = 1;
		}
...
static int check_db_local(void) {
	...
	db = klpm_get_localdb(config->handle);
	pkglist = klpm_db_get_pkgcache(db);
...
static int check_db_sync(void) {
	int ret = 0;
	klpm_list_t *i, *dblist, *pkglist, *syncpkglist = NULL;

	dblist = klpm_get_syncdbs(config->handle);
	for(i = dblist; i; i = klpm_list_next(i)) {
		pkglist = klpm_db_get_pkgcache(i->data);
		syncpkglist = klpm_list_join(syncpkglist, klpm_list_copy(pkglist));
	}
	ret = check_db_missing_deps(syncpkglist);

	klpm_list_free(syncpkglist);
	return ret;
}
...
int kuzpkg_database(klpm_list_t *targets)
{
	...
	if(config->flags & (KUZPKG_TRANS_FLAG_ALLDEPS | KUZPKG_TRANS_FLAG_ALLEXPLICIT)) {
