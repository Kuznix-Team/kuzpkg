...
static struct kuzpkg_progress_bar *totalbar;

/* delayed output during progress bar */
static int on_progress = 0;
static klpm_list_t *output = NULL;
...
struct kuzpkg_multibar_ui {
	 ...
	 */
	klpm_list_t *active_downloads; /* List of type 'struct kuzpkg_progress_bar' */
...
struct kuzpkg_multibar_ui multibar_ui = {0};

static int dload_progressbar_enabled(void);
static void init_total_progressbar(void);
static void update_bar_finalstats(struct kuzpkg_progress_bar *bar);
static void draw_kuzpkg_progress_bar(struct kuzpkg_progress_bar *bar);
...
/* callback to handle messages/notifications from libkuzpkg transactions */
void cb_event(void *ctx, klpm_event_t *event)
{
	...
	switch(event->type) {
		case KUZPKG_EVENT_HOOK_START:
			if(event->hook.when == KUZPKG_HOOK_PRE_TRANSACTION) {
				colon_printf(_("Running pre-transaction hooks...\n"));
			} else {
				colon_printf(_("Running post-transaction hooks...\n"));
			}
			break;
		case KUZPKG_EVENT_HOOK_RUN_START:
			{
				klpm_event_hook_run_t *e = &event->hook_run;
			...
			break;
		case KUZPKG_EVENT_CHECKDEPS_START:
			printf(_("checking dependencies...\n"));
			break;
		case KUZPKG_EVENT_FILECONFLICTS_START:
			if(config->noprogressbar) {
				printf(_("checking for file conflicts...\n"));
			}
			break;
		case KUZPKG_EVENT_RESOLVEDEPS_START:
			printf(_("resolving dependencies...\n"));
			break;
		case KUZPKG_EVENT_INTERCONFLICTS_START:
			printf(_("looking for conflicting packages...\n"));
			break;
		case KUZPKG_EVENT_TRANSACTION_START:
			colon_printf(_("Processing package changes...\n"));
			break;
		case KUZPKG_EVENT_PACKAGE_OPERATION_START:
			if(config->noprogressbar) {
				klpm_event_package_operation_t *e = &event->package_operation;
				switch(e->operation) {
					case KUZPKG_PACKAGE_INSTALL:
						printf(_("installing %s...\n"), klpm_pkg_get_name(e->newpkg));
						break;
					...
					case KUZPKG_PACKAGE_DOWNGRADE:
						...
						break;
					case KUZPKG_PACKAGE_REMOVE:
						printf(_("removing %s...\n"), klpm_pkg_get_name(e->oldpkg));
						break;
				}
			}
			break;
		case KUZPKG_EVENT_PACKAGE_OPERATION_DONE:
			{
				klpm_event_package_operation_t *e = &event->package_operation;
				switch(e->operation) {
					case KUZPKG_PACKAGE_INSTALL:
			...
			break;
		case KUZPKG_EVENT_INTEGRITY_START:
			if(config->noprogressbar) {
				printf(_("checking package integrity...\n"));
			}
			break;
		case KUZPKG_EVENT_KEYRING_START:
			if(config->noprogressbar) {
				printf(_("checking keyring...\n"));
			}
			break;
		case KUZPKG_EVENT_KEY_DOWNLOAD_START:
			printf(_("downloading required keys...\n"));
			break;
		case KUZPKG_EVENT_LOAD_START:
			if(config->noprogressbar) {
				printf(_("loading package files...\n"));
			}
			break;
		case KUZPKG_EVENT_SCRIPTLET_INFO:
			fputs(event->scriptlet_info.line, stdout);
			break;
		case KUZPKG_EVENT_DB_RETRIEVE_START:
			on_progress = 1;
			break;
		case KUZPKG_EVENT_PKG_RETRIEVE_START:
			...
			break;
		case KUZPKG_EVENT_DISKSPACE_START:
			if(config->noprogressbar) {
				printf(_("checking available disk space...\n"));
			}
			break;
		case KUZPKG_EVENT_OPTDEP_REMOVAL:
			{
				klpm_event_optdep_removal_t *e = &event->optdep_removal;
				char *dep_string = klpm_dep_compute_string(e->optdep);
				colon_printf(_("%s optionally requires %s\n"),
						klpm_pkg_get_name(e->pkg),
						dep_string);
				free(dep_string);
			}
			break;
		case KUZPKG_EVENT_DATABASE_MISSING:
			if(!config->op_s_sync) {
			...
			break;
		case KUZPKG_EVENT_PACNEW_CREATED:
			{
				klpm_event_pacnew_created_t *e = &event->pacnew_created;
				if(on_progress) {
					char *string = NULL;
					pm_sprintf(&string, KUZPKG_LOG_WARNING, _("%s installed as %s.pacnew\n"),
							e->file, e->file);
					if(string != NULL) {
						output = klpm_list_add(output, string);
					}
				} else {
					pm_printf(KUZPKG_LOG_WARNING, _("%s installed as %s.pacnew\n"),
							e->file, e->file);
				}
			}
			break;
		case KUZPKG_EVENT_PACSAVE_CREATED:
			{
				klpm_event_pacsave_created_t *e = &event->pacsave_created;
				if(on_progress) {
					char *string = NULL;
					pm_sprintf(&string, KUZPKG_LOG_WARNING, _("%s saved as %s.pacsave\n"),
							e->file, e->file);
					if(string != NULL) {
						output = klpm_list_add(output, string);
					}
				} else {
					pm_printf(KUZPKG_LOG_WARNING, _("%s saved as %s.pacsave\n"),
							e->file, e->file);
				}
			}
			break;
		case KUZPKG_EVENT_DB_RETRIEVE_DONE:
		case KUZPKG_EVENT_DB_RETRIEVE_FAILED:
		case KUZPKG_EVENT_PKG_RETRIEVE_DONE:
		case KUZPKG_EVENT_PKG_RETRIEVE_FAILED:
			console_cursor_move_end();
			if(total_enabled) {
				update_bar_finalstats(totalbar);
				draw_kuzpkg_progress_bar(totalbar);
		...
		case KUZPKG_EVENT_FILECONFLICTS_DONE:
		case KUZPKG_EVENT_CHECKDEPS_DONE:
		case KUZPKG_EVENT_RESOLVEDEPS_DONE:
		case KUZPKG_EVENT_INTERCONFLICTS_DONE:
		case KUZPKG_EVENT_TRANSACTION_DONE:
		case KUZPKG_EVENT_INTEGRITY_DONE:
		case KUZPKG_EVENT_KEYRING_DONE:
		case KUZPKG_EVENT_KEY_DOWNLOAD_DONE:
		case KUZPKG_EVENT_LOAD_DONE:
		case KUZPKG_EVENT_DISKSPACE_DONE:
		case KUZPKG_EVENT_HOOK_DONE:
		case KUZPKG_EVENT_HOOK_RUN_DONE:
			/* nothing */
			break;
	}
	fflush(stdout);
...
/* callback to handle questions from libkuzpkg transactions (yes/no) */
void cb_question(void *ctx, klpm_question_t *question)
{
	...
	if(config->print) {
		switch(question->type) {
			case KUZPKG_QUESTION_INSTALL_IGNOREPKG:
			case KUZPKG_QUESTION_REPLACE_PKG:
	...
	switch(question->type) {
		case KUZPKG_QUESTION_INSTALL_IGNOREPKG:
			{
				klpm_question_install_ignorepkg_t *q = &question->install_ignorepkg;
				if(!config->op_s_downloadonly) {
					q->install = yesno(_("%s is in IgnorePkg/IgnoreGroup. Install anyway?"),
							klpm_pkg_get_name(q->pkg));
				} else {
					q->install = 1;
				}
			}
		...
		case KUZPKG_QUESTION_REPLACE_PKG:
			{
				klpm_question_replace_t *q = &question->replace;
				q->replace = yesno(_("Replace %s with %s/%s?"),
						klpm_pkg_get_name(q->oldpkg),
						klpm_db_get_name(q->newdb),
						klpm_pkg_get_name(q->newpkg));
			}
			break;
		case KUZPKG_QUESTION_CONFLICT_PKG:
			{
				klpm_question_conflict_t *q = &question->conflict;
				/* print conflict only if it contains new information */
				if(strcmp(klpm_pkg_get_name(q->conflict->package1), q->conflict->reason->name) == 0
						|| strcmp(klpm_pkg_get_name(q->conflict->package2), q->conflict->reason->name) == 0) {
					q->remove = noyes(_("%s-%s%s%s and %s-%s%s%s are in conflict. Remove %s?"),
							klpm_pkg_get_name(q->conflict->package1),
							colstr->faint,
							klpm_pkg_get_version(q->conflict->package1),
							colstr->nocolor,
							klpm_pkg_get_name(q->conflict->package2),
							colstr->faint,
							klpm_pkg_get_version(q->conflict->package2),
							colstr->nocolor,
							klpm_pkg_get_name(q->conflict->package2));
				} else {
			...
			break;
		case KUZPKG_QUESTION_REMOVE_PKGS:
			{
				klpm_question_remove_pkgs_t *q = &question->remove_pkgs;
				klpm_list_t *namelist = NULL, *i;
				size_t count = 0;
				for(i = q->packages; i; i = i->next) {
					namelist = klpm_list_add(namelist,
							(char *)klpm_pkg_get_name(i->data));
					count++;
				}
				colon_printf(_n(
				...
				q->skip = noyes(_n(
							...
							count));
				klpm_list_free(namelist);
			}
			break;
		case KUZPKG_QUESTION_SELECT_PROVIDER:
			{
				klpm_question_select_provider_t *q = &question->select_provider;
				size_t count = klpm_list_count(q->providers);
				char *depstring = klpm_dep_compute_string(q->depend);
			...
			break;
		case KUZPKG_QUESTION_CORRUPTED_PKG:
			{
				klpm_question_corrupted_t *q = &question->corrupted;
				q->remove = yesno(_("File %s is corrupted (%s).\n"
							"Do you want to delete it?"),
						q->filepath,
						klpm_strerror(q->reason));
			}
			break;
		case KUZPKG_QUESTION_IMPORT_KEY:
			{
				klpm_question_import_key_t *q = &question->import_key;
		...
		int percent, size_t howmany, size_t current)
{
	...
	switch(event) {
		case KUZPKG_PROGRESS_ADD_START:
			opr = _("installing");
			break;
		case KUZPKG_PROGRESS_UPGRADE_START:
			opr = _("upgrading");
			break;
		case KUZPKG_PROGRESS_DOWNGRADE_START:
			opr = _("downgrading");
			break;
		case KUZPKG_PROGRESS_REINSTALL_START:
			opr = _("reinstalling");
			break;
		case KUZPKG_PROGRESS_REMOVE_START:
			opr = _("removing");
			break;
		case KUZPKG_PROGRESS_CONFLICTS_START:
			opr = _("checking for file conflicts");
			break;
		case KUZPKG_PROGRESS_DISKSPACE_START:
			opr = _("checking available disk space");
			break;
		case KUZPKG_PROGRESS_INTEGRITY_START:
			opr = _("checking package integrity");
			break;
		case KUZPKG_PROGRESS_KEYRING_START:
			opr = _("checking keys in keyring");
			break;
		case KUZPKG_PROGRESS_LOAD_START:
...
static bool find_bar_for_filename(const char *filename, int *index, struct kuzpkg_progress_bar **bar)
{
	int i = 0;
	klpm_list_t *listitem = multibar_ui.active_downloads;
	for(; listitem; listitem = listitem->next, i++) {
		struct kuzpkg_progress_bar *b = listitem->data;
...
static void init_total_progressbar(void)
{
	totalbar = calloc(1, sizeof(struct kuzpkg_progress_bar));
	assert(totalbar);
...
static void dload_init_event(const char *filename, klpm_download_event_init_t *data)
{
	(void)data;
	...
	struct kuzpkg_progress_bar *bar = calloc(1, sizeof(struct kuzpkg_progress_bar));
	assert(bar);
	bar->filename = strdup(filename);
	assert(bar->filename);
	bar->init_time = get_time_ms();
	bar->rate = 0.0;
	multibar_ui.active_downloads = klpm_list_add(multibar_ui.active_downloads, bar);
 ...
 */
static bool update_bar_stats(struct kuzpkg_progress_bar *bar)
{
	int64_t timediff;
...
static void dload_progress_event(const char *filename, klpm_download_event_progress_t *data)
{
	int index;
	struct kuzpkg_progress_bar *bar;
	bool ok;
	off_t last_chunk_amount;
...
/* download retried */
static void dload_retry_event(const char *filename, klpm_download_event_retry_t *data) {
	if(!dload_progressbar_enabled()) {
		return;
	}
...
/* download completed */
static void dload_complete_event(const char *filename, klpm_download_event_completed_t *data)
{
	int index;
	struct kuzpkg_progress_bar *bar;
	bool ok;
	 ...
	 * an klpm_download_event_progress_t event happened */
	bar->total_size = data->total;
	...
	if(data->result == 1) {
		...
		if(multibar_ui.move_completed_up && index != 0) {
			 ...
			 */
			struct kuzpkg_progress_bar *former_topbar = multibar_ui.active_downloads->data;
			klpm_list_t *baritem = klpm_list_nth(multibar_ui.active_downloads, index);
			multibar_ui.active_downloads->data = bar;
			baritem->data = former_topbar;

			console_cursor_goto_bar(index);
			draw_kuzpkg_progress_bar(former_topbar);
	...
	while(multibar_ui.active_downloads) {
		klpm_list_t *head = multibar_ui.active_downloads;
		struct kuzpkg_progress_bar *j = head->data;
		if(j->completed) {
			multibar_ui.cursor_lineno--;
			multibar_ui.active_downloads_num--;
			multibar_ui.active_downloads = klpm_list_remove_item(
...
void cb_download(void *ctx, const char *filename, klpm_download_event_type_t event, void *data)
{
	(void)ctx;
	...
	if(event == KUZPKG_DOWNLOAD_INIT) {
		dload_init_event(filename, data);
	} else if(event == KUZPKG_DOWNLOAD_PROGRESS) {
		dload_progress_event(filename, data);
	} else if(event == KUZPKG_DOWNLOAD_RETRY) {
		dload_retry_event(filename, data);
	} else if(event == KUZPKG_DOWNLOAD_COMPLETED) {
		dload_complete_event(filename, data);
	} else {
		pm_printf(KUZPKG_LOG_ERROR, _("unknown callback event type %d for %s\n"),
				event, filename);
	}
}
