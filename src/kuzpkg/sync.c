...
static int unlink_verbose(const char *pathname, int ignore_missing)
{
	...
	if(ret) {
		if(ignore_missing && errno == ENOENT) {
			...
			pm_printf(KUZPKG_LOG_ERROR, _("could not remove %s: %s\n"),
...
static int sync_cleandb(const char *dbpath)
{
	...
	dir = opendir(dbpath);
	if(dir == NULL) {
		pm_printf(KUZPKG_LOG_ERROR, _("could not access database directory\n"));
		return 1;
	}

	syncdbs = klpm_get_syncdbs(config->handle);
	...
	while((ent = readdir(dir)) != NULL) {
		...
		if(len >= PATH_MAX) {
			pm_printf(KUZPKG_LOG_ERROR, _("could not remove %s%s: path exceeds PATH_MAX\n"),
					dbpath, dname);
		}
		...
		for(i = syncdbs; i && !found; i = klpm_list_next(i)) {
			klpm_db_t *db = i->data;
			found = !strcmp(dbname, klpm_db_get_name(db));
		}
...
static int sync_cleandb_all(void)
{
	...
	dbpath = klpm_option_get_dbpath(config->handle);
...
static int sync_cleancache(int level)
{
	...
	klpm_list_t *cachedirs = klpm_option_get_cachedirs(config->handle);
	int ret = 0;
	...
	for(i = cachedirs; i; i = klpm_list_next(i)) {
		...
		while((ent = readdir(dir)) != NULL) {
			char path[PATH_MAX];
			int delete = 1;
			klpm_pkg_t *localpkg = NULL, *pkg = NULL;
			...
			if(len >= PATH_MAX) {
				pm_printf(KUZPKG_LOG_ERROR, _("skipping %s%s: path exceeds PATH_MAX\n"),
						cachedir, ent->d_name);
				continue;
			}
			 ...
			 * just the metadata. */
			if(klpm_pkg_load(config->handle, path, 0, 0, &localpkg) != 0) {
				pm_printf(KUZPKG_LOG_DEBUG, "skipping %s, could not load as package\n",
						path);
				continue;
			}
			local_name = klpm_pkg_get_name(localpkg);
			local_version = klpm_pkg_get_version(localpkg);

			if(config->cleanmethod & PM_CLEAN_KEEPINST) {
				/* check if this package is in the local DB */
				pkg = klpm_db_get_pkg(db_local, local_name);
				if(pkg != NULL && klpm_pkg_vercmp(local_version,
							klpm_pkg_get_version(pkg)) == 0) {
					/* package was found in local DB and version matches, keep it */
					pm_printf(KUZPKG_LOG_DEBUG, "package %s-%s found in local db\n",
			...
			if(config->cleanmethod & PM_CLEAN_KEEPCUR) {
				...
				for(j = sync_dbs; j && delete; j = klpm_list_next(j)) {
					klpm_db_t *db = j->data;
					pkg = klpm_db_get_pkg(db, local_name);
					if(pkg != NULL && klpm_pkg_vercmp(local_version,
								klpm_pkg_get_version(pkg)) == 0) {
						/* package was found in a sync DB and version matches, keep it */
						pm_printf(KUZPKG_LOG_DEBUG, "package %s-%s found in sync db\n",
								local_name, local_version);
						delete = 0;
					}
				}
...
static int sync_search(klpm_list_t *syncs, klpm_list_t *targets)
{
	klpm_list_t *i;
	int found = 0;
...
static int sync_group(int level, klpm_list_t *syncs, klpm_list_t *targets)
{
	klpm_list_t *i, *j, *k, *s = NULL;
	int ret = 0;
	...
	if(targets) {
		...
		for(i = targets; i; i = klpm_list_next(i)) {
			...
			for(j = syncs; j; j = klpm_list_next(j)) {
				klpm_db_t *db = j->data;
				klpm_group_t *grp = klpm_db_get_group(db, grpname);

				if(grp) {
					found++;
					/* get names of packages in group */
					for(k = grp->packages; k; k = klpm_list_next(k)) {
						if(!config->quiet) {
							printf("%s %s\n", grpname,
									klpm_pkg_get_name(k->data));
						} else {
							printf("%s\n", klpm_pkg_get_name(k->data));
						}
					}
		...
		for(i = syncs; i; i = klpm_list_next(i)) {
			...
			for(j = klpm_db_get_groupcache(db); j; j = klpm_list_next(j)) {
				...
				if(level > 1) {
					...
					if(!klpm_list_find_str (s, grp->name)) {
						s = klpm_list_add (s, grp->name);
						printf("%s\n", grp->name);
					}
				}
			}
...
static int sync_info(klpm_list_t *syncs, klpm_list_t *targets)
{
	klpm_list_t *i, *j, *k;
	int ret = 0;
	...
	if(targets) {
		for(i = targets; i; i = klpm_list_next(i)) {
			...
			for(j = syncs; j; j = klpm_list_next(j)) {
				klpm_db_t *db = j->data;
				if(repo && strcmp(repo, klpm_db_get_name(db)) != 0) {
					continue;
				}
				founddb = 1;

				for(k = klpm_db_get_pkgcache(db); k; k = klpm_list_next(k)) {
					klpm_pkg_t *pkg = k->data;

					if(strcmp(klpm_pkg_get_name(pkg), pkgstr) == 0) {
...
static int sync_list(klpm_list_t *syncs, klpm_list_t *targets)
{
	klpm_list_t *i, *j, *ls = NULL;
	klpm_db_t *db_local = klpm_get_localdb(config->handle);
	int ret = 0;
	...
	if(targets) {
		for(i = targets; i; i = klpm_list_next(i)) {
			...
			for(j = syncs; j; j = klpm_list_next(j)) {
				...
				if(strcmp(repo, klpm_db_get_name(d)) == 0) {
					db = d;
					break;
				}
			}
			...
			ls = klpm_list_add(ls, db);
		}
	} else {
		ls = syncs;
	}

	for(i = ls; i; i = klpm_list_next(i)) {
		klpm_db_t *db = i->data;
...
static klpm_db_t *get_db(const char *dbname)
{
	klpm_list_t *i;
	for(i = klpm_get_syncdbs(config->handle); i; i = i->next) {
		klpm_db_t *db = i->data;
...
static int process_pkg(klpm_pkg_t *pkg)
{
	int ret = klpm_add_pkg(config->handle, pkg);

	if(ret == -1) {
		klpm_errno_t err = klpm_errno(config->handle);
		pm_printf(KUZPKG_LOG_ERROR, "'%s': %s\n", klpm_pkg_get_name(pkg), klpm_strerror(err));
		return 1;
	}
	config->explicit_adds = klpm_list_add(config->explicit_adds, pkg);
	return 0;
}

static int group_exists(klpm_list_t *dbs, const char *name)
{
	klpm_list_t *i;
	for(i = dbs; i; i = i->next) {
		klpm_db_t *db = i->data;

		if(klpm_db_get_group(db, name)) {
			return 1;
		}
	}
...
static int process_group(klpm_list_t *dbs, const char *group, int error)
{
	int ret = 0;
	klpm_list_t *i;
	klpm_list_t *pkgs = klpm_find_group_pkgs(dbs, group);
	int count = klpm_list_count(pkgs);
	...
	if(!count) {
		...
		pm_printf(KUZPKG_LOG_ERROR, _("target not found: %s\n"), group);
		return 1;
	}
	...
	if(config->print == 0) {
		...
		for(i = pkgs, n = 0; i; i = klpm_list_next(i)) {
		...
		for(i = pkgs; i; i = klpm_list_next(i)) {
			klpm_pkg_t *pkg = i->data;
...
static int process_targname(klpm_list_t *dblist, const char *targname,
		int error)
{
	klpm_pkg_t *pkg = klpm_find_dbs_satisfier(config->handle, dblist, targname);

	/* skip ignored packages when user says no */
	if(klpm_errno(config->handle) == KUZPKG_ERR_PKG_IGNORED) {
			pm_printf(KUZPKG_LOG_WARNING, _("skipping target: %s\n"), targname);
			return 0;
	}
...
static int process_target(const char *target, int error)
{
	...
	if(targname && targname != targstring) {
		...
		*targname = '\0';
		...
		db = get_db(dbname);
		if(!db) {
			pm_printf(KUZPKG_LOG_ERROR, _("database not found: %s\n"),
					dbname);
			ret = 1;
			goto cleanup;
		}
		...
		klpm_db_set_usage(db, usage|KUZPKG_DB_USAGE_INSTALL);

		dblist = klpm_list_add(NULL, db);
		ret = process_targname(dblist, targname, error);
		klpm_list_free(dblist);

		/* restore old usage so we don't possibly disturb later
		 * targets */
		klpm_db_set_usage(db, usage);
	} else {
		targname = targstring;
		dblist = klpm_get_syncdbs(config->handle);
		ret = process_targname(dblist, targname, error);
	}
...
static int sync_trans(klpm_list_t *targets)
{
	int retval = 0;
	klpm_list_t *i;
	...
	if(config->op_s_upgrade) {
		...
		if(klpm_sync_sysupgrade(config->handle, config->op_s_upgrade >= 2) == -1) {
			pm_printf(KUZPKG_LOG_ERROR, "%s\n", klpm_strerror(klpm_errno(config->handle)));
			trans_release();
			return 1;
		}
	}
...
static void print_broken_dep(klpm_depmissing_t *miss)
{
	char *depstring = klpm_dep_compute_string(miss->depend);
	klpm_list_t *trans_add = klpm_trans_get_add(config->handle);
	klpm_pkg_t *pkg;
	...
	} else if((pkg = klpm_pkg_find(trans_add, miss->causingpkg))) {
		/* upgrading a package breaks a local dependency */
		colon_printf(_("installing %s (%s) breaks dependency '%s' required by %s\n"),
				miss->causingpkg, klpm_pkg_get_version(pkg), depstring, miss->target);
	} else {
...
{
	klpm_list_t *i, *packages, *data = NULL;
	int retval = 0;

	/* Step 2: "compute" the transaction based on targets and flags */
	if(klpm_trans_prepare(config->handle, &data) == -1) {
		klpm_errno_t err = klpm_errno(config->handle);
		pm_printf(KUZPKG_LOG_ERROR, _("failed to prepare transaction (%s)\n"),
		        klpm_strerror(err));
		switch(err) {
			case KUZPKG_ERR_PKG_INVALID_ARCH:
				for(i = data; i; i = klpm_list_next(i)) {
					char *pkg = i->data;
			...
			case KUZPKG_ERR_CONFLICTING_DEPS:
				for(i = data; i; i = klpm_list_next(i)) {
					...
					if(conflict->reason->mod == KUZPKG_DEP_MOD_ANY) {
						colon_printf(_("%s-%s and %s-%s are in conflict\n"),
								klpm_pkg_get_name(conflict->package1),
								klpm_pkg_get_version(conflict->package1),
								klpm_pkg_get_name(conflict->package2),
								klpm_pkg_get_version(conflict->package2));
					} else {
						char *reason = klpm_dep_compute_string(conflict->reason);
	...
	packages = klpm_trans_get_add(config->handle);
	if(packages == NULL) {
	...
	if(klpm_trans_commit(config->handle, &data) == -1) {
		klpm_errno_t err = klpm_errno(config->handle);
		pm_printf(KUZPKG_LOG_ERROR, _("failed to commit transaction (%s)\n"),
		        klpm_strerror(err));
		switch(err) {
			case KUZPKG_ERR_FILE_CONFLICTS:
				for(i = data; i; i = klpm_list_next(i)) {
					klpm_fileconflict_t *conflict = i->data;
					switch(conflict->type) {
						case KUZPKG_FILECONFLICT_TARGET:
							fprintf(stderr, _("%s exists in both '%s' and '%s'\n"),
									conflict->file, conflict->target, conflict->ctarget);
							break;
						case KUZPKG_FILECONFLICT_FILESYSTEM:
...
int kuzpkg_sync(klpm_list_t *targets)
{
	...
	sync_dbs = klpm_get_syncdbs(config->handle);
