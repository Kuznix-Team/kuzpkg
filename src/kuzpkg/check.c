		...
		struct stat *st)
{
	...
	if(llstat(filepath, st) != 0) {
		if(klpm_option_match_noextract(config->handle, filepath + rootlen) == 0) {
			/* NoExtract */
			return -1;
		} else {
			if(config->quiet) {
				...
				pm_printf(KUZPKG_LOG_WARNING, "%s: %s (%s)\n",
		...
		struct stat *st, struct archive_entry *entry)
{
	...
	if((archive_type == AE_IFREG && !S_ISREG(file_type)) ||
		...
		if(config->quiet) {
			...
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (File type mismatch)\n"),
					pkgname, filepath);
		}
		return 1;
	}
		...
		struct stat *st, struct archive_entry *entry)
{
	...
	if(st->st_uid != archive_entry_uid(entry)) {
		errors++;
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (UID mismatch)\n"),
					pkgname, filepath);
		}
	}
	...
	if(st->st_gid != archive_entry_gid(entry)) {
		errors++;
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (GID mismatch)\n"),
					pkgname, filepath);
		}
	}
	...
	if(fsmode != (~AE_IFMT & archive_entry_mode(entry))) {
		errors++;
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (Permissions mismatch)\n"),
					pkgname, filepath);
		}
	}
		...
		struct stat *st, struct archive_entry *entry, int backup)
{
	if(st->st_mtime != archive_entry_mtime(entry)) {
		...
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (Modification time mismatch)\n"),
					pkgname, filepath);
		}
		return 1;
	}
		...
		struct stat *st, struct archive_entry *entry)
{
	...
	if(readlink(filepath, link, length) != st->st_size) {
		...
		pm_printf(KUZPKG_LOG_ERROR, _("unable to read symlink contents: %s\n"), filepath);
		return 1;
	}
	link[length - 1] = '\0';
	...
	if(strcmp(link, archive_entry_symlink(entry)) != 0) {
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (Symlink path mismatch)\n"),
					pkgname, filepath);
		}
		return 1;
	}
		...
		struct stat *st, struct archive_entry *entry, int backup)
{
	if(st->st_size != archive_entry_size(entry)) {
		...
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (Size mismatch)\n"),
					pkgname, filepath);
		}
		return 1;
	}
		...
		int backup, const char *cksum_name, const char *cksum_calc, const char *cksum_mtree)
{
	if(!cksum_calc) {
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (failed to calculate %s checksum)\n"),
	...
	if(!cksum_mtree) {
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (%s checksum information not available)\n"),
					pkgname, filepath, cksum_name);
		}
		return 1;
	}
	...
	if(strcmp(cksum_calc, cksum_mtree) != 0) {
		...
		if(!config->quiet) {
			pm_printf(KUZPKG_LOG_WARNING, _("%s: %s (%s checksum mismatch)\n"),
					pkgname, filepath, cksum_name);
		}
		return 1;
	}
		...
		struct archive_entry *entry, int backup)
{
	int errors = 0;
#if ARCHIVE_VERSION_NUMBER >= 3005000
	char *cksum_calc = klpm_compute_sha256sum(filepath);
...
int check_pkg_fast(klpm_pkg_t *pkg)
{
	...
	if(rootlen + 1 > PATH_MAX) {
		...
		pm_printf(KUZPKG_LOG_ERROR, _("path too long: %s%s\n"), root, "");
		return 1;
	}
	strcpy(filepath, root);

	pkgname = klpm_pkg_get_name(pkg);
	filelist = klpm_pkg_get_files(pkg);
	for(i = 0; i < filelist->count; i++) {
		...
		if(rootlen + 1 + plen > PATH_MAX) {
			pm_printf(KUZPKG_LOG_WARNING, _("path too long: %s%s\n"), root, path);
			continue;
		}
...
int check_pkg_full(klpm_pkg_t *pkg)
{
	const char *root, *pkgname;
	size_t errors = 0;
	size_t rootlen;
	...
	while(klpm_pkg_mtree_next(pkg, mtree, &entry) == 0) {
		struct stat st;
		...
		if(*path == '.') {
			...
			/* Do not append root directory as klpm_option_get_dbpath is already
			 * an absolute path */
			filepath_len = snprintf(filepath, PATH_MAX, "%slocal/%s-%s/%s",
					klpm_option_get_dbpath(config->handle),
					pkgname, klpm_pkg_get_version(pkg), dbfile);
			if(filepath_len >= PATH_MAX) {
				pm_printf(KUZPKG_LOG_WARNING, _("path too long: %slocal/%s-%s/%s\n"),
						klpm_option_get_dbpath(config->handle),
						pkgname, klpm_pkg_get_version(pkg), dbfile);
				continue;
			}
		...
		if(type != AE_IFDIR && type != AE_IFREG && type != AE_IFLNK) {
			pm_printf(KUZPKG_LOG_WARNING, _("file type not recognized: %s%s\n"), root, path);
			continue;
		}
	...
	klpm_pkg_mtree_close(pkg, mtree);
