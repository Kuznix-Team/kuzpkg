...
config_t *config_new(void)
{
	...
	newconfig->logmask = KUZPKG_LOG_ERROR | KUZPKG_LOG_WARNING;
	newconfig->configfile = strdup(CONFFILE);
	if(klpm_capabilities() & KUZPKG_CAPABILITY_SIGNATURES) {
		newconfig->siglevel = KUZPKG_SIG_PACKAGE | KUZPKG_SIG_DATABASE;
		newconfig->localfilesiglevel = KUZPKG_SIG_USE_DEFAULT;
		newconfig->remotefilesiglevel = KUZPKG_SIG_USE_DEFAULT;
	}
...
int config_free(config_t *oldconfig)
{
	...
	klpm_list_free(oldconfig->explicit_adds);
	klpm_list_free(oldconfig->explicit_removes);

	klpm_list_free_inner(config->repos, (klpm_list_fn_free) config_repo_free);
	klpm_list_free(config->repos);
...
static int systemvp(const char *file, char *const argv[])
{
	...
	if(pid == 0) {
		...
		if (config->sandboxuser) {
			ret = klpm_sandbox_setup_child(config->handle, config->sandboxuser, NULL, false);
			if (ret != 0) {
				pm_printf(KUZPKG_LOG_ERROR, _("switching to sandbox user '%s' failed!\n"), config->sandboxuser);
				_Exit(ret);
			}
		}
		...
		const char *localpath, int force)
{
	...
	if(cwdfd < 0) {
		pm_printf(KUZPKG_LOG_ERROR, _("could not get current working directory\n"));
	}

	/* cwd to the download directory */
	if(chdir(localpath)) {
		pm_printf(KUZPKG_LOG_WARNING, _("could not chdir to download directory %s\n"), localpath);
		ret = -1;
		goto cleanup;
	}
	...
	if(config->logmask & KUZPKG_LOG_DEBUG) {
		...
		if(cmd) {
			pm_printf(KUZPKG_LOG_DEBUG, "running command: %s\n", cmd);
			free(cmd);
		}
	}
	...
	if(retval == -1) {
		pm_printf(KUZPKG_LOG_WARNING, _("running XferCommand: fork failed!\n"));
		ret = -1;
	} else if(retval != 0) {
		/* download failed */
		pm_printf(KUZPKG_LOG_DEBUG, "XferCommand command returned non-zero status "
				"code (%d)\n", retval);
		ret = -1;
	} else {
		...
		if(usepart) {
			if(rename(tempfile, destfile)) {
				pm_printf(KUZPKG_LOG_ERROR, _("could not rename %s to %s (%s)\n"),
...
cleanup:
	...
	if(cwdfd >= 0) {
		if(fchdir(cwdfd) != 0) {
			pm_printf(KUZPKG_LOG_ERROR, _("could not restore working directory (%s)\n"),
					strerror(errno));
		}
		close(cwdfd);
	}
...
int config_add_architecture(char *arch)
{
	...
	pm_printf(KUZPKG_LOG_DEBUG, "config: arch: %s\n", arch);
	config->architectures = klpm_list_add(config->architectures, arch);
	return 0;
}
 ...
 */
static int process_siglevel(klpm_list_t *values, int *storage,
...
#define SLUNSET(sl) do { level &= ~(sl); mask |= (sl); } while(0)
	...
	for(i = values; i; i = klpm_list_next(i)) {
		...
		if(strcmp(value, "Never") == 0) {
			if(package) {
				SLUNSET(KUZPKG_SIG_PACKAGE);
			}
			if(database) {
				SLUNSET(KUZPKG_SIG_DATABASE);
			}
		} else if(strcmp(value, "Optional") == 0) {
			if(package) {
				SLSET(KUZPKG_SIG_PACKAGE | KUZPKG_SIG_PACKAGE_OPTIONAL);
			}
			if(database) {
				SLSET(KUZPKG_SIG_DATABASE | KUZPKG_SIG_DATABASE_OPTIONAL);
			}
		} else if(strcmp(value, "Required") == 0) {
			if(package) {
				SLSET(KUZPKG_SIG_PACKAGE);
				SLUNSET(KUZPKG_SIG_PACKAGE_OPTIONAL);
			}
			if(database) {
				SLSET(KUZPKG_SIG_DATABASE);
				SLUNSET(KUZPKG_SIG_DATABASE_OPTIONAL);
			}
		} else if(strcmp(value, "TrustedOnly") == 0) {
			if(package) {
				SLUNSET(KUZPKG_SIG_PACKAGE_MARGINAL_OK | KUZPKG_SIG_PACKAGE_UNKNOWN_OK);
			}
			if(database) {
				SLUNSET(KUZPKG_SIG_DATABASE_MARGINAL_OK | KUZPKG_SIG_DATABASE_UNKNOWN_OK);
			}
		} else if(strcmp(value, "TrustAll") == 0) {
			if(package) {
				SLSET(KUZPKG_SIG_PACKAGE_MARGINAL_OK | KUZPKG_SIG_PACKAGE_UNKNOWN_OK);
			}
			if(database) {
				SLSET(KUZPKG_SIG_DATABASE_MARGINAL_OK | KUZPKG_SIG_DATABASE_UNKNOWN_OK);
			}
		} else {
			pm_printf(KUZPKG_LOG_ERROR,
...
#undef SLUNSET
	...
	if(!(klpm_capabilities() & KUZPKG_CAPABILITY_SIGNATURES) &&
			level & (KUZPKG_SIG_PACKAGE | KUZPKG_SIG_DATABASE)) {
		pm_printf(KUZPKG_LOG_ERROR,
...
static int process_cleanmethods(klpm_list_t *values,
...
/** Add repeating options such as NoExtract, NoUpgrade, etc to libkuzpkg
		...
		klpm_list_t **list)
{
	...
	while(val) {
		*list = klpm_list_add(*list, strdup(val));
		pm_printf(KUZPKG_LOG_DEBUG, "config: %s: %s\n", option, val);
		...
		const char *file, int linenum)
{
	if(value == NULL) {
		...
		if(strcmp(key, "UseSyslog") == 0) {
			config->usesyslog = 1;
			pm_printf(KUZPKG_LOG_DEBUG, "config: usesyslog\n");
		} else if(strcmp(key, "ILoveCandy") == 0) {
			config->chomp = 1;
			pm_printf(KUZPKG_LOG_DEBUG, "config: chomp\n");
		} else if(strcmp(key, "VerbosePkgLists") == 0) {
			config->verbosepkglists = 1;
			pm_printf(KUZPKG_LOG_DEBUG, "config: verbosepkglists\n");
		...
		if(strcmp(key, "NoUpgrade") == 0) {
			...
			*/
			...
			klpm_list_t *i, *arches = NULL;
			setrepeatingoption(value, "Architecture", &arches);
			for(i = arches; i; i = klpm_list_next(i)) {
				config_add_architecture(i->data);
			}
			klpm_list_free(arches);
			...
			if(!config->dbpath) {
				...
				pm_printf(KUZPKG_LOG_DEBUG, "config: dbpath: %s\n", path);
			}
			...
			if(!config->rootdir) {
				...
				pm_printf(KUZPKG_LOG_DEBUG, "config: rootdir: %s\n", path);
			}
			...
			if(!config->gpgdir) {
				...
				pm_printf(KUZPKG_LOG_DEBUG, "config: gpgdir: %s\n", path);
			}
		} else if(strcmp(key, "LogFile") == 0) {
			if(!config->logfile) {
				config->logfile = strdup(value);
				pm_printf(KUZPKG_LOG_DEBUG, "config: logfile: %s\n", value);
			}
		} else if(strcmp(key, "DownloadUser") == 0) {
			if(!config->sandboxuser) {
				config->sandboxuser = strdup(value);
				pm_printf(KUZPKG_LOG_DEBUG, "config: sandboxuser: %s\n", value);
			}
			...
			pm_printf(KUZPKG_LOG_DEBUG, "config: xfercommand: %s\n", value);
		} else if(strcmp(key, "CleanMethod") == 0) {
			klpm_list_t *methods = NULL;
...
static char *replace_server_vars(config_t *c, config_repo_t *r, const char *s)
{
	...
	if(c->architectures) {
		char *temp, *replaced;
		klpm_list_t *i = config->architectures;
...
static int replace_server_list_vars(config_t *c, config_repo_t *r, klpm_list_t *list)
{
	klpm_list_t *i;
...
static int register_repo(config_repo_t *repo)
{
	...
	db = klpm_register_syncdb(config->handle, repo->name, repo->siglevel);
	if(db == NULL) {
		pm_printf(KUZPKG_LOG_ERROR, _("could not register '%s' database (%s)\n"),
				repo->name, klpm_strerror(klpm_errno(config->handle)));
		return 1;
	}

	pm_printf(KUZPKG_LOG_DEBUG, "setting usage of %d for %s repository\n",
			repo->usage, repo->name);
	klpm_db_set_usage(db, repo->usage);

	for(i = repo->cache_servers; i; i = klpm_list_next(i)) {
		const char *value = i->data;
		if(klpm_db_add_cache_server(db, value) != 0) {
			/* pm_errno is set by klpm_db_setserver */
			pm_printf(KUZPKG_LOG_ERROR, _("could not add cache server URL to database '%s': %s (%s)\n"),
					klpm_db_get_name(db), value, klpm_strerror(klpm_errno(config->handle)));
			return 1;
		}
	}
	for(i = repo->servers; i; i = klpm_list_next(i)) {
		const char *value = i->data;
		if(klpm_db_add_server(db, value) != 0) {
			/* pm_errno is set by klpm_db_setserver */
			pm_printf(KUZPKG_LOG_ERROR, _("could not add server URL to database '%s': %s (%s)\n"),
					klpm_db_get_name(db), value, klpm_strerror(klpm_errno(config->handle)));
			return 1;
		}
	}
...
static int setup_libkuzpkg(void)
{
	...
	pm_printf(KUZPKG_LOG_DEBUG, "setup_libkuzpkg called\n");

	/* initialize library */
	handle = klpm_initialize(config->rootdir, config->dbpath, &err);
	if(!handle) {
		pm_printf(KUZPKG_LOG_ERROR, _("failed to initialize klpm library:\n(root: %s, dbpath: %s)\n%s\n"),
		        config->rootdir, config->dbpath, klpm_strerror(err));
		if(err == KUZPKG_ERR_DB_VERSION) {
			fprintf(stderr, _("try running kuzpkg-db-upgrade\n"));
		}
		return -1;
	}
	config->handle = handle;

	klpm_option_set_logcb(handle, cb_log, NULL);
	klpm_option_set_dlcb(handle, cb_download, NULL);
	klpm_option_set_eventcb(handle, cb_event, NULL);
	klpm_option_set_questioncb(handle, cb_question, NULL);
	klpm_option_set_progresscb(handle, cb_progress, NULL);

	if(config->op == PM_OP_FILES) {
		klpm_option_set_dbext(handle, ".files");
	}

	ret = klpm_option_set_logfile(handle, config->logfile);
	if(ret != 0) {
		pm_printf(KUZPKG_LOG_ERROR, _("problem setting logfile '%s' (%s)\n"),
				config->logfile, klpm_strerror(klpm_errno(handle)));
		return ret;
	}

	/* Set GnuPG's home directory. This is not relative to rootdir, even if
	 * rootdir is defined. Reasoning: gpgdir contains configuration data. */
	ret = klpm_option_set_gpgdir(handle, config->gpgdir);
	if(ret != 0) {
		pm_printf(KUZPKG_LOG_ERROR, _("problem setting gpgdir '%s' (%s)\n"),
				config->gpgdir, klpm_strerror(klpm_errno(handle)));
		return ret;
	}
	...
	for(i = config->hookdirs; i; i = klpm_list_next(i)) {
		if((ret = klpm_option_add_hookdir(handle, i->data)) != 0) {
			pm_printf(KUZPKG_LOG_ERROR, _("problem adding hookdir '%s' (%s)\n"),
					(char *) i->data, klpm_strerror(klpm_errno(handle)));
			return ret;
		}
	}

	klpm_option_set_cachedirs(handle, config->cachedirs);

	klpm_option_set_overwrite_files(handle, config->overwrite_files);

	klpm_option_set_default_siglevel(handle, config->siglevel);

	klpm_option_set_local_file_siglevel(handle, config->localfilesiglevel);
	klpm_option_set_remote_file_siglevel(handle, config->remotefilesiglevel);

	for(i = config->repos; i; i = klpm_list_next(i)) {
		register_repo(i->data);
	}

	if(config->xfercommand) {
		klpm_option_set_fetchcb(handle, download_with_xfercommand, NULL);
	} else if(!(klpm_capabilities() & KUZPKG_CAPABILITY_DOWNLOADER)) {
		pm_printf(KUZPKG_LOG_WARNING, _("no '%s' configured\n"), "XferCommand");
	}

	klpm_option_set_architectures(handle, config->architectures);
	klpm_option_set_checkspace(handle, config->checkspace);
	klpm_option_set_usesyslog(handle, config->usesyslog);

	if((ret = klpm_option_set_sandboxuser(handle, config->sandboxuser)) != 0) {
		pm_printf(KUZPKG_LOG_ERROR, _("problem setting DownloadUser '%s' (user does not exist)\n"),
					config->sandboxuser);
		return ret;
	}
	klpm_option_set_disable_sandbox_filesystem(handle, config->disable_sandbox_filesystem);
	klpm_option_set_disable_sandbox_syscalls(handle, config->disable_sandbox_syscalls);

	klpm_option_set_ignorepkgs(handle, config->ignorepkg);
	klpm_option_set_ignoregroups(handle, config->ignoregrp);
	klpm_option_set_noupgrades(handle, config->noupgrade);
	klpm_option_set_noextracts(handle, config->noextract);

	klpm_option_set_disable_dl_timeout(handle, config->disable_dl_timeout);
	klpm_option_set_parallel_downloads(handle, config->parallel_downloads);

	for(i = config->assumeinstalled; i; i = i->next) {
		char *entry = i->data;
		klpm_depend_t *dep = klpm_dep_from_string(entry);
		if(!dep) {
			return 1;
		}
		pm_printf(KUZPKG_LOG_DEBUG, "parsed assume installed: %s %s\n", dep->name, dep->version);

		ret = klpm_option_add_assumeinstalled(handle, dep);
		klpm_dep_free(dep);
		if(ret) {
			pm_printf(KUZPKG_LOG_ERROR, _("Failed to pass %s entry to libkuzpkg"), "assume-installed");
			return ret;
		}
	 }
...
static int process_usage(klpm_list_t *values, int *usage,
		const char *file, int linenum)
{
	klpm_list_t *i;
	int level = *usage;
	...
	for(i = values; i; i = i->next) {
		...
		if(strcmp(key, "Sync") == 0) {
			level |= KUZPKG_DB_USAGE_SYNC;
		} else if(strcmp(key, "Search") == 0) {
			level |= KUZPKG_DB_USAGE_SEARCH;
		} else if(strcmp(key, "Install") == 0) {
			level |= KUZPKG_DB_USAGE_INSTALL;
		} else if(strcmp(key, "Upgrade") == 0) {
			level |= KUZPKG_DB_USAGE_UPGRADE;
		} else if(strcmp(key, "All") == 0) {
			level |= KUZPKG_DB_USAGE_ALL;
		} else {
			pm_printf(KUZPKG_LOG_ERROR,
...
#define CHECK_VALUE(val) do { \
	if(!val) { \
		pm_printf(KUZPKG_LOG_ERROR, _("config file %s, line %d: directive '%s' needs a value\n"), \
				file, line, key); \
		return 1; \
	} \
} while(0)
	...
	if(strcmp(key, "CacheServer") == 0) {
		CHECK_VALUE(value);
		repo->cache_servers = klpm_list_add(repo->cache_servers, strdup(value));
	} else if(strcmp(key, "Server") == 0) {
		CHECK_VALUE(value);
		repo->servers = klpm_list_add(repo->servers, strdup(value));
	} else if(strcmp(key, "SigLevel") == 0) {
		CHECK_VALUE(value);
		klpm_list_t *values = NULL;
		setrepeatingoption(value, "SigLevel", &values);
	...
	switch(globret) {
		...
		default:
			for(gindex = 0; gindex < globbuf.gl_pathc; gindex++) {
				pm_printf(KUZPKG_LOG_DEBUG, "config file %s, line %d: including %s\n",
		...
		char *key, char *value, void *data)
{
	struct section_t *section = data;
	if(!name && !key && !value) {
		pm_printf(KUZPKG_LOG_ERROR, _("config file %s could not be read: %s\n"),
				file, strerror(errno));
		return 1;
	} else if(!key && !value) {
		section->name = name;
		pm_printf(KUZPKG_LOG_DEBUG, "config: new section '%s'\n", name);
		if(strcmp(name, "options") == 0) {
			section->repo = NULL;
		} else {
			section->repo = calloc(sizeof(config_repo_t), 1);
			section->repo->name = strdup(name);
			section->repo->siglevel = KUZPKG_SIG_USE_DEFAULT;
			section->repo->usage = 0;
			config->repos = klpm_list_add(config->repos, section->repo);
		}
		return 0;
	}
	...
	if(section->name == NULL) {
		pm_printf(KUZPKG_LOG_ERROR, _("config file %s, line %d: All directives must belong to a section.\n"),
				file, linenum);
		return 1;
	}
...
#define SETDEFAULT(opt, val) if(!opt) { opt = val; if(!opt) { return -1; } }
	...
	SETDEFAULT(c->cachedirs, klpm_list_add(NULL, strdup(CACHEDIR)));
	SETDEFAULT(c->hookdirs, klpm_list_add(NULL, strdup(HOOKDIR)));
	...
	for(i = c->repos; i; i = i->next) {
		config_repo_t *r = i->data;
		SETDEFAULT(r->usage, KUZPKG_DB_USAGE_ALL);
...
int parseconfig(const char *file)
{
	...
	pm_printf(KUZPKG_LOG_DEBUG, "config: finished parsing %s\n", file);
	if((ret = setup_libkuzpkg())) {
		return ret;
	}
	klpm_list_free_inner(config->repos, (klpm_list_fn_free) config_repo_free);
	klpm_list_free(config->repos);
