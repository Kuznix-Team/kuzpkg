...
{
#define addlist(s) (list = klpm_list_add(list, s))
	klpm_list_t *list = NULL, *i;
	...
	list = klpm_list_msort(list, klpm_list_count(list), options_cmp);
...
/** Output kuzpkg version and copyright.
 */
static void version(void)
{
        printf("\n");
        printf("Kuzpkg v%s - libkuzpkg v%s\n", PACKAGE_VERSION, klpm_version());
        printf("Copyright (C) 2026 Kuznix\n");
        printf("\n");
...
static void setuseragent(void)
{
	...
	uname(&un);
	len = snprintf(agent, 100, "kuzpkg/%s (%s %s) libkuzpkg/%s",
			PACKAGE_VERSION, un.sysname, un.machine, klpm_version());
	if(len >= 100) {
		pm_printf(KUZPKG_LOG_WARNING, _("HTTP_USER_AGENT truncated\n"));
	}
...
static void cleanup(int ret)
{
	...
	if(config) {
		/* free klpm library resources */
		if(config->handle && klpm_release(config->handle) == -1) {
			pm_printf(KUZPKG_LOG_ERROR, "error releasing klpm library\n");
		}
...
static int parsearg_util_addlist(klpm_list_t **list)
{
	char *i, *save = NULL;

	for(i = strtok_r(optarg, ",", &save); i; i = strtok_r(NULL, ",", &save)) {
		*list = klpm_list_add(*list, strdup(i));
	}
	...
	switch(opt) {
		...
		case OP_ASK:
			if(optarg) {
				...
				if(errno == ERANGE || endptr == optarg || *endptr != '\0' || ask > UINT_MAX) {
					pm_printf(KUZPKG_LOG_ERROR, "'%s' is not a valid ask level\n",
							optarg);
					return 2;
				}
				...
				pm_printf(KUZPKG_LOG_ERROR, "no value provided for ask level\n");
				return 2;
			}
			break;
		case OP_CACHEDIR:
			{
				...
				if(path != NULL) {
					config->cachedirs = klpm_list_add(config->cachedirs, path);
				} else {
					return 2;
				}
			}
		...
		case OP_COLOR:
			if(strcmp("never", optarg) == 0) {
				...
				pm_printf(KUZPKG_LOG_ERROR, _("invalid argument '%s' for %s\n"),
						optarg, "--color");
				return 2;
			}
		...
		case OP_DEBUG:
			...
			if(optarg) {
				...
				if(errno == ERANGE || endptr == optarg || *endptr != '\0') {
					pm_printf(KUZPKG_LOG_ERROR, _("'%s' is not a valid debug level\n"),
							optarg);
					return 2;
				}
				...
				switch(debug) {
					case 2:
						config->logmask |= KUZPKG_LOG_FUNCTION;
						__attribute__((fallthrough));
					case 1:
						config->logmask |= KUZPKG_LOG_DEBUG;
						break;
					default:
						pm_printf(KUZPKG_LOG_ERROR, _("'%s' is not a valid debug level\n"),
								optarg);
		...
		case OP_HOOKDIR:
			{
				...
				if(path != NULL) {
					config->hookdirs = klpm_list_add(config->hookdirs, path);
				} else {
					return 2;
				}
			}
...
static int parsearg_database(int opt)
{
	switch(opt) {
		case OP_ASDEPS:
			config->flags |= KUZPKG_TRANS_FLAG_ALLDEPS;
			break;
		case OP_ASEXPLICIT:
			config->flags |= KUZPKG_TRANS_FLAG_ALLEXPLICIT;
			break;
		case OP_CHECK:
		case 'k':
			(config->op_q_check)++;
...
{
	invalid_opt(config->flags & KUZPKG_TRANS_FLAG_ALLDEPS
			&& config->flags & KUZPKG_TRANS_FLAG_ALLEXPLICIT,
			"--asdeps", "--asexplicit");
	...
	if(config->op_q_check) {
		invalid_opt(config->flags & KUZPKG_TRANS_FLAG_ALLDEPS,
				"--asdeps", "--check");
		invalid_opt(config->flags & KUZPKG_TRANS_FLAG_ALLEXPLICIT,
				"--asexplicit", "--check");
	}
}
...
static int parsearg_trans(int opt)
{
	switch(opt) {
		case OP_NODEPS:
		case 'd':
			if(config->flags & KUZPKG_TRANS_FLAG_NODEPVERSION) {
				config->flags |= KUZPKG_TRANS_FLAG_NODEPS;
			} else {
				config->flags |= KUZPKG_TRANS_FLAG_NODEPVERSION;
			}
			break;
		case OP_DBONLY:
			config->flags |= KUZPKG_TRANS_FLAG_DBONLY;
			config->flags |= KUZPKG_TRANS_FLAG_NOSCRIPTLET;
			config->flags |= KUZPKG_TRANS_FLAG_NOHOOKS;
			break;
...
{
	if(config->print) {
		invalid_opt(config->flags & KUZPKG_TRANS_FLAG_DBONLY,
				"--print", "--dbonly");
		invalid_opt(config->flags & KUZPKG_TRANS_FLAG_NOSCRIPTLET,
				"--print", "--noscriptlet");
	}
}
...
static int parsearg_remove(int opt)
{
	...
	switch(opt) {
		case OP_CASCADE:
		case 'c':
			config->flags |= KUZPKG_TRANS_FLAG_CASCADE;
			break;
		case OP_NOSAVE:
		case 'n':
			config->flags |= KUZPKG_TRANS_FLAG_NOSAVE;
			break;
		case OP_RECURSIVE:
		case 's':
			if(config->flags & KUZPKG_TRANS_FLAG_RECURSE) {
				config->flags |= KUZPKG_TRANS_FLAG_RECURSEALL;
			} else {
				config->flags |= KUZPKG_TRANS_FLAG_RECURSE;
			}
			break;
		case OP_UNNEEDED:
		case 'u':
			config->flags |= KUZPKG_TRANS_FLAG_UNNEEDED;
			break;
		default:
			return 1;
	}
...
{
	checkargs_trans();
	if(config->flags & KUZPKG_TRANS_FLAG_NOSAVE) {
...
static int parsearg_upgrade(int opt)
{
	...
	switch(opt) {
		...
		case OP_ASEXPLICIT:
			...
			break;
		case OP_NEEDED:
			config->flags |= KUZPKG_TRANS_FLAG_NEEDED;
			break;
		case OP_IGNORE:
		...
		case 'w':
			...
			config->flags |= KUZPKG_TRANS_FLAG_DOWNLOADONLY;
			config->flags |= KUZPKG_TRANS_FLAG_NOCONFLICTS;
			break;
		default: return 1;
	}
	return 0;
...
static int parseargs(int argc, char *argv[])
{
	...
	if(config->op == 0) {
		pm_printf(KUZPKG_LOG_ERROR, _("only one operation may be used at a time\n"));
		return 1;
	}
	if(config->help) {
		...
		if(result != 0) {
			...
			/* global option parsing failed, abort */
				if(opt < OP_LONG_FLAG_MIN) {
					pm_printf(KUZPKG_LOG_ERROR, _("invalid option '-%c'\n"), opt);
				} else {
					pm_printf(KUZPKG_LOG_ERROR, _("invalid option '--%s'\n"),
	...
	while(optind < argc) {
		...
		pm_targets = klpm_list_add(pm_targets, strdup(argv[optind]));
		optind++;
	}
...
int main(int argc, char *argv[])
{
	...
	if(myuid > 0 && needs_root()) {
		pm_printf(KUZPKG_LOG_ERROR, _("you cannot perform this operation unless you are root.\n"));
		cleanup(EXIT_FAILURE);
	}

	/* we support reading targets from stdin if a cmdline parameter is '-' */
	if(klpm_list_find_str(pm_targets, "-")) {
		if(!isatty(fileno(stdin))) {
			...
			pm_targets = klpm_list_remove_str(pm_targets, "-", &vdata);
			free(vdata);
			...
			while((nread = getline(&line, &line_size, stdin)) != -1) {
				...
				if(!klpm_list_append_strdup(&pm_targets, line)) {
					break;
				}
				target_found = 1;
			}
			...
			if(!freopen(ctermid(NULL), "r", stdin)) {
				pm_printf(KUZPKG_LOG_ERROR, _("failed to reopen stdin for reading: (%s)\n"),
						strerror(errno));
			}

			if(!target_found) {
				pm_printf(KUZPKG_LOG_ERROR, _("argument '-' specified with empty stdin\n"));
				cleanup(1);
			}
		} else {
			/* do not read stdin from terminal */
			pm_printf(KUZPKG_LOG_ERROR, _("argument '-' specified without input on stdin\n"));
			cleanup(1);
		}
	}

	pm_printf(KUZPKG_LOG_DEBUG, "kuzpkg v%s - libkuzpkg v%s\n", PACKAGE_VERSION, klpm_version());
	...
	if(config->print && !config->op_s_clean) {
		config->noconfirm = 1;
		config->flags |= KUZPKG_TRANS_FLAG_NOCONFLICTS;
		config->flags |= KUZPKG_TRANS_FLAG_NOLOCK;
		/* Display only errors */
		config->logmask &= ~KUZPKG_LOG_WARNING;
	}

	if(config->verbose > 0) {
		klpm_list_t *j;
		printf("Root      : %s\n", klpm_option_get_root(config->handle));
		printf("Conf File : %s\n", config->configfile);
		printf("DB Path   : %s\n", klpm_option_get_dbpath(config->handle));
		printf("Cache Dirs: ");
		for(j = klpm_option_get_cachedirs(config->handle); j; j = klpm_list_next(j)) {
			printf("%s  ", (const char *)j->data);
		}
		printf("\n");
		printf("Hook Dirs : ");
		for(j = klpm_option_get_hookdirs(config->handle); j; j = klpm_list_next(j)) {
			printf("%s  ", (const char *)j->data);
		}
		printf("\n");
		printf("Lock File : %s\n", klpm_option_get_lockfile(config->handle));
		printf("Log File  : %s\n", klpm_option_get_logfile(config->handle));
		printf("GPG Dir   : %s\n", klpm_option_get_gpgdir(config->handle));
	...
	switch(config->op) {
		case PM_OP_DATABASE:
			ret = kuzpkg_database(pm_targets);
			break;
		case PM_OP_REMOVE:
			ret = kuzpkg_remove(pm_targets);
			break;
		case PM_OP_UPGRADE:
			ret = kuzpkg_upgrade(pm_targets);
			break;
		case PM_OP_QUERY:
			ret = kuzpkg_query(pm_targets);
			break;
		case PM_OP_SYNC:
			ret = kuzpkg_sync(pm_targets);
			break;
		case PM_OP_DEPTEST:
			ret = kuzpkg_deptest(pm_targets);
			break;
		case PM_OP_FILES:
			ret = kuzpkg_files(pm_targets);
			break;
		default:
			pm_printf(KUZPKG_LOG_ERROR, _("no operation specified (use -h for help)\n"));
			ret = EXIT_FAILURE;
	}
