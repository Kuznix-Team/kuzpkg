 ...
 */
static void *_package_changelog_open(klpm_pkg_t *pkg)
{
	...
	fd = _klpm_open_archive(pkg->handle, pkgfile, &buf,
	...
	while(archive_read_next_header(archive, &entry) == ARCHIVE_OK) {
		...
		if(strcmp(entry_name, ".CHANGELOG") == 0) {
			...
			if(!changelog) {
				pkg->handle->pm_errno = KUZPKG_ERR_MEMORY;
		...
		const klpm_pkg_t UNUSED *pkg, void *fp)
{
	...
	if(sret < 0) {
		RET_ERR(pkg->handle, KUZPKG_ERR_LIBARCHIVE, 0);
	} else {
		return (size_t)sret;
	}
}
 ...
 */
static int _package_changelog_close(const klpm_pkg_t UNUSED *pkg, void *fp)
{
	int ret;
	struct package_changelog *changelog = fp;
	ret = _klpm_archive_read_free(changelog->archive);
	close(changelog->fd);
	free(changelog);
	return ret;
}
...
/**
 * Parses the package description file for a package into a klpm_pkg_t struct.
 * @param archive the archive to read from, pointed at the .PKGINFO entry
 * @param newpkg an empty klpm_pkg_t struct to fill with package info
 *
 * @return 0 on success, -1 on error
 */
static int parse_descfile(klpm_handle_t *handle, struct archive *a, klpm_pkg_t *newpkg)
{
	char *ptr = NULL;
	char *key = NULL;
	int ret, linenum = 0;
	...
	while((ret = _klpm_archive_fgets(a, &buf)) == ARCHIVE_OK) {
		size_t len = _klpm_strip_newline(buf.line, buf.real_line_size);
		...
		if(!ptr || (size_t)(ptr - key + 2) > len || memcmp(ptr, " = ", 3) != 0) {
			...
			*ptr = '\0';
			...
			if(strcmp(key, "pkgname") == 0) {
				...
				newpkg->name_hash = _klpm_hash_sdbm(newpkg->name);
				...
				newpkg->groups = klpm_list_add(newpkg->groups, tmp);
			} else if(strcmp(key, "url") == 0) {
				STRDUP(newpkg->url, ptr, return -1);
			} else if(strcmp(key, "license") == 0) {
				char *tmp = NULL;
				STRDUP(tmp, ptr, return -1);
				newpkg->licenses = klpm_list_add(newpkg->licenses, tmp);
			} else if(strcmp(key, "builddate") == 0) {
				newpkg->builddate = _klpm_parsedate(ptr);
				...
				newpkg->isize = _klpm_strtoofft(ptr);
			} else if(strcmp(key, "depend") == 0) {
				klpm_depend_t *dep = klpm_dep_from_string(ptr);
				newpkg->depends = klpm_list_add(newpkg->depends, dep);
			} else if(strcmp(key, "optdepend") == 0) {
				klpm_depend_t *optdep = klpm_dep_from_string(ptr);
				newpkg->optdepends = klpm_list_add(newpkg->optdepends, optdep);
			} else if(strcmp(key, "makedepend") == 0) {
				klpm_depend_t *makedep = klpm_dep_from_string(ptr);
				newpkg->makedepends = klpm_list_add(newpkg->makedepends, makedep);
			} else if(strcmp(key, "checkdepend") == 0) {
				klpm_depend_t *checkdep = klpm_dep_from_string(ptr);
				newpkg->checkdepends = klpm_list_add(newpkg->checkdepends, checkdep);
			} else if(strcmp(key, "conflict") == 0) {
				klpm_depend_t *conflict = klpm_dep_from_string(ptr);
				newpkg->conflicts = klpm_list_add(newpkg->conflicts, conflict);
			} else if(strcmp(key, "replaces") == 0) {
				klpm_depend_t *replace = klpm_dep_from_string(ptr);
				newpkg->replaces = klpm_list_add(newpkg->replaces, replace);
			} else if(strcmp(key, "provides") == 0) {
				klpm_depend_t *provide = klpm_dep_from_string(ptr);
				newpkg->provides = klpm_list_add(newpkg->provides, provide);
			} else if(strcmp(key, "backup") == 0) {
				klpm_backup_t *backup;
				CALLOC(backup, 1, sizeof(klpm_backup_t), return -1);
				STRDUP(backup->name, ptr, FREE(backup); return -1);
				newpkg->backup = klpm_list_add(newpkg->backup, backup);
			} else if(strcmp(key, "xdata") == 0) {
				klpm_pkg_xdata_t *pd = _klpm_pkg_parse_xdata(ptr);
				if(pd == NULL || !klpm_list_append(&newpkg->xdata, pd)) {
					_klpm_pkg_xdata_free(pd);
					return -1;
				}
			} else {
				const char *pkgname = newpkg->name ? newpkg->name : "error";
				_klpm_log(handle, KUZPKG_LOG_WARNING, _("%s: unknown key '%s' in package description\n"), pkgname, key);
				_klpm_log(handle, KUZPKG_LOG_DEBUG, "%s: unknown key '%s' in description file line %d\n",
									pkgname, key, linenum);
			}
		}
	}
	if(ret != ARCHIVE_EOF) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "error parsing package descfile\n");
		return -1;
	}
		...
		klpm_siglist_t **sigdata, int *validation)
{
	...
	if(_klpm_access(handle, NULL, pkgfile, R_OK) != 0) {
		if(errno == ENOENT) {
			handle->pm_errno = KUZPKG_ERR_PKG_NOT_FOUND;
		} else if(errno == EACCES) {
			handle->pm_errno = KUZPKG_ERR_BADPERMS;
		} else {
			handle->pm_errno = KUZPKG_ERR_PKG_OPEN;
		}
		return -1;
	}
	...
	if(level & KUZPKG_SIG_PACKAGE) {
		if(syncpkg && syncpkg->base64_sig) {
			...
			char *sigpath = _klpm_sigpath(handle, pkgfile);
			if(sigpath && !_klpm_access(handle, NULL, sigpath, R_OK)) {
				has_sig = 1;
			}
			free(sigpath);
		}
	...
	if(syncpkg && (!has_sig || !syncpkg->base64_sig)) {
		if(syncpkg->sha256sum) {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "sha256sum: %s\n", syncpkg->sha256sum);
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "checking sha256sum for %s\n", pkgfile);
			if(_klpm_test_checksum(pkgfile, syncpkg->sha256sum, KUZPKG_PKG_VALIDATION_SHA256SUM) != 0) {
				RET_ERR(handle, KUZPKG_ERR_PKG_INVALID_CHECKSUM, -1);
			}
			if(validation) {
				*validation |= KUZPKG_PKG_VALIDATION_SHA256SUM;
			}
		}
	}
	...
	if(level & KUZPKG_SIG_PACKAGE) {
		...
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "sig data: %s\n", sig ? sig : "<from .sig>");
		if(!has_sig && !(level & KUZPKG_SIG_PACKAGE_OPTIONAL)) {
			handle->pm_errno = KUZPKG_ERR_PKG_MISSING_SIG;
			return -1;
		}
		if(_klpm_check_pgp_helper(handle, pkgfile, sig,
					level & KUZPKG_SIG_PACKAGE_OPTIONAL, level & KUZPKG_SIG_PACKAGE_MARGINAL_OK,
					level & KUZPKG_SIG_PACKAGE_UNKNOWN_OK, sigdata)) {
			handle->pm_errno = KUZPKG_ERR_PKG_INVALID_SIG;
			return -1;
		}
		if(validation && has_sig) {
			*validation |= KUZPKG_PKG_VALIDATION_SIGNATURE;
		}
	}

	if(validation && !*validation) {
		*validation = KUZPKG_PKG_VALIDATION_NONE;
	}
...
/**
 * Handle the existence of simple paths for _klpm_load_pkg_internal()
 * @param pkg package to change
 * @param path path to examine
 * @return 0 if path doesn't match any rule, 1 if it has been handled
 */
static int handle_simple_path(klpm_pkg_t *pkg, const char *path)
{
 ...
 */
static int add_entry_to_files_list(klpm_filelist_t *filelist,
		size_t *files_size, struct archive_entry *entry, const char *path)
{
	const size_t files_count = filelist->count;
	klpm_file_t *current_file;
	mode_t type;
	size_t pathlen;

	if(!_klpm_greedy_grow((void **)&filelist->files,
				files_size, (files_count + 1) * sizeof(klpm_file_t))) {
		return -1;
	}
 ...
 */
static int build_filelist_from_mtree(klpm_handle_t *handle, klpm_pkg_t *pkg, struct archive *archive)
{
	int ret = 0;
	size_t i;
	...
	if((mtree = archive_read_new()) == NULL) {
		GOTO_ERR(handle, KUZPKG_ERR_LIBARCHIVE, error);
	}
	...
	while(1) {
		...
		if(!_klpm_greedy_grow((void **)&mtree_data, &mtree_maxsize, mtree_cursize + KUZPKG_BUFFER_SIZE)) {
			goto error;
		}

		size = archive_read_data(archive, mtree_data + mtree_cursize, KUZPKG_BUFFER_SIZE);

		if(size < 0) {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, _("error while reading package %s: %s\n"),
	...
	if(ret != ARCHIVE_EOF && ret != ARCHIVE_OK) { /* An error occurred */
		_klpm_log(handle, KUZPKG_LOG_DEBUG, _("error while reading mtree of package %s: %s\n"),
				pkg->filename, archive_error_string(mtree));
		GOTO_ERR(handle, KUZPKG_ERR_LIBARCHIVE, error);
	}
	...
	memcpy(&pkg->files, &filelist, sizeof(klpm_filelist_t));

	free(mtree_data);
	_klpm_archive_read_free(mtree);
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "finished mtree reading for %s\n", pkg->filename);
	return 0;
error:
...
/**
 * Load a package and create the corresponding klpm_pkg_t struct.
 * @param handle the context handle
 * @param pkgfile path to the package file
		...
		const char *pkgfile, int full)
{
	...
	fd = _klpm_open_archive(handle, pkgfile, &st, &archive, KUZPKG_ERR_PKG_OPEN);
	if(fd < 0) {
		if(errno == ENOENT) {
			handle->pm_errno = KUZPKG_ERR_PKG_NOT_FOUND;
		} else if(errno == EACCES) {
	...
	newpkg = _klpm_pkg_new();
	if(newpkg == NULL) {
		GOTO_ERR(handle, KUZPKG_ERR_MEMORY, error);
	}
	STRDUP(newpkg->filename, pkgfile, GOTO_ERR(handle, KUZPKG_ERR_MEMORY, error));
	newpkg->size = st.st_size;

	_klpm_log(handle, KUZPKG_LOG_DEBUG, "starting package load for %s\n", pkgfile);
	...
	while((ret = archive_read_next_header(archive, &entry)) == ARCHIVE_OK) {
		...
		if(strcmp(entry_name, ".PKGINFO") == 0) {
			...
			if(parse_descfile(handle, archive, newpkg) != 0) {
				_klpm_log(handle, KUZPKG_LOG_ERROR, _("could not parse package description file in %s\n"),
						pkgfile);
				goto pkg_invalid;
			}
			if(newpkg->name == NULL || strlen(newpkg->name) == 0) {
				_klpm_log(handle, KUZPKG_LOG_ERROR, _("missing package name in %s\n"), pkgfile);
				goto pkg_invalid;
			}
			if(newpkg->version == NULL || strlen(newpkg->version) == 0) {
				_klpm_log(handle, KUZPKG_LOG_ERROR, _("missing package version in %s\n"), pkgfile);
				goto pkg_invalid;
			}
			if(strchr(newpkg->version, '-') == NULL) {
				_klpm_log(handle, KUZPKG_LOG_ERROR, _("invalid package version in %s\n"), pkgfile);
				goto pkg_invalid;
			}
			config = 1;
			continue;
		...
		if(archive_read_data_skip(archive)) {
			_klpm_log(handle, KUZPKG_LOG_ERROR, _("error while reading package %s: %s\n"),
	...
	if(!config) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("missing package metadata in %s\n"), pkgfile);
		goto pkg_invalid;
	}

	/* internal fields for package struct */
	newpkg->origin = KUZPKG_PKG_FROM_FILE;
	STRDUP(newpkg->origin_data.file, pkgfile, goto error);
	newpkg->ops = get_file_pkg_ops();
	newpkg->handle = handle;
	newpkg->infolevel = INFRQ_BASE | INFRQ_DESC | INFRQ_SCRIPTLET;
	newpkg->validation = KUZPKG_PKG_VALIDATION_NONE;

	if(full) {
		if(newpkg->files.files) {
			/* attempt to hand back any memory we don't need */
			REALLOC(newpkg->files.files, sizeof(klpm_file_t) * newpkg->files.count, (void)0);
			/* "checking for conflicts" requires a sorted list, ensure that here */
			_klpm_log(handle, KUZPKG_LOG_DEBUG,
					"sorting package filelist for %s\n", pkgfile);

			_klpm_filelist_sort(&newpkg->files);
		}
		newpkg->infolevel |= INFRQ_FILES;
	}

	if(_klpm_pkg_check_meta(newpkg) != 0) {
		goto pkg_invalid;
	}
...
pkg_invalid:
	handle->pm_errno = KUZPKG_ERR_PKG_INVALID;
error:
...
int SYMEXPORT klpm_pkg_load(klpm_handle_t *handle, const char *filename, int full,
		int level, klpm_pkg_t **pkg)
{
	...
	sigpath = _klpm_sigpath(handle, filename);
	if(sigpath && !_klpm_access(handle, NULL, sigpath, R_OK)) {
		if(level & KUZPKG_SIG_PACKAGE) {
			...
			if(klpm_extract_keyid(handle, filename, sig, len, &keys) == 0) {
				klpm_list_t *k;
				for(k = keys; k; k = k->next) {
					char *key = k->data;
					if(_klpm_key_in_keychain(handle, key) == 0) {
						pkg_temp = _klpm_pkg_load_internal(handle, filename, full);
						if(_klpm_key_import(handle, NULL, key) == -1) {
							fail = 1;
						}
						_klpm_pkg_free(pkg_temp);
					}
				}
				FREELIST(keys);
			}
	...
	if(_klpm_pkg_validate_internal(handle, filename, NULL, level, NULL,
				&validation) == -1) {
		/* pm_errno is set by pkg_validate */
		return -1;
	}
	*pkg = _klpm_pkg_load_internal(handle, filename, full);
	if(*pkg == NULL) {
