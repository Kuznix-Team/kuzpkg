...
bool _klpm_sandbox_syscalls_filter(klpm_handle_t *handle)
{
	...
	if(ctx == NULL) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("error initializing seccomp to filter system calls in the download sandbox!\n"));
		return false;
	}
	...
	for(idx = 0; idx < sizeof(denied_syscalls) / sizeof(*denied_syscalls); idx++) {
		...
		if(syscall != __NR_SCMP_ERROR) {
			if(seccomp_rule_add(ctx, SCMP_ACT_ERRNO(EPERM), syscall, 0) != 0) {
				_klpm_log(handle, KUZPKG_LOG_ERROR, _("error restricting syscall %s via seccomp!\n"), denied_syscalls[idx]);
			}
			else {
	...
	if(seccomp_load(ctx) != 0) {
		ret = errno;
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("error restricting syscalls via seccomp: %d!\n"), ret);
	}
	else {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, _("successfully restricted %d syscalls via seccomp\n"), restrictedSyscallsCount);
	}
