...
static int _klpm_log_leader(FILE *f, const char *prefix)
{
	time_t t = time(NULL);
...
int SYMEXPORT klpm_logaction(klpm_handle_t *handle, const char *prefix,
		const char *fmt, ...)
{
	int ret = 0;
	va_list args;
	...
	if(handle->logstream == NULL && handle->logfile != NULL) {
		...
		if(fd < 0 || (handle->logstream = fdopen(fd, "a")) == NULL) {
			if(errno == EACCES) {
				...
				handle->pm_errno = KUZPKG_ERR_SYSTEM;
			}
			ret = -1;
		}
	}
	...
	if(handle->logstream) {
		if(_klpm_log_leader(handle->logstream, prefix) < 0
...
void _klpm_log(klpm_handle_t *handle, klpm_loglevel_t flag, const char *fmt, ...)
{
	va_list args;
