...
static FILE *create_tempfile(struct dload_payload *payload, const char *localpath)
{
	...
	MALLOC(randpath, len, RET_ERR(payload->handle, KUZPKG_ERR_MEMORY, NULL));
	snprintf(randpath, len, "%sklpmtmp.XXXXXX", localpath);
	if((fd = mkstemp(randpath)) == -1 ||
			fchmod(fd, ~(_getumask()) & 0666) ||
			!(fp = fdopen(fd, payload->tempfile_openmode))) {
		unlink(randpath);
		close(fd);
		_klpm_log(payload->handle, KUZPKG_LOG_ERROR,
				_("failed to create temporary file for download\n"));
		free(randpath);
		return NULL;
	}
	/* fp now points to our klpmtmp.XXXXXX */
	free(payload->tempfile_name);
	payload->tempfile_name = randpath;
	free(payload->remote_name);
	STRDUP(payload->remote_name, strrchr(randpath, '/') + 1,
			fclose(fp); RET_ERR(payload->handle, KUZPKG_ERR_MEMORY, NULL));
...
static int curl_add_payload(klpm_handle_t *handle, CURLM *curlm,
...
static struct server_error_count *find_server_errors(klpm_handle_t *handle, const char *server)
{
	klpm_list_t *i;
	...
	if((h = calloc(sizeof(struct server_error_count), 1))
			&& klpm_list_append(&handle->server_errors, h)) {
...
static int should_skip_server(klpm_handle_t *handle, const char *server)
{
	struct server_error_count *h;
...
static int should_skip_cache_server(klpm_handle_t *handle, const char *server)
{
	struct server_error_count *h;
	if(server_error_limit && (h = find_server_errors(handle, server)) ) {
		return h->errors < 0;
...
static void server_soft_error(klpm_handle_t *handle, const char *server)
{
	struct server_error_count *h;
...
static void server_hard_error(klpm_handle_t *handle, const char *server)
{
	struct server_error_count *h;
	if(server_error_limit && (h = find_server_errors(handle, server))) {
		if(h->errors != -1) {
		...
		curl_off_t UNUSED ultotal, curl_off_t UNUSED ulnow)
{
	...
	klpm_download_event_progress_t cb_data = {0};
	...
	payload->handle->dlcb(payload->handle->dlcb_ctx,
			payload->remote_name, KUZPKG_DOWNLOAD_PROGRESS, &cb_data);
...
{
	klpm_handle_t *handle = payload->handle;
	const char *useragent = getenv("HTTP_USER_AGENT");
	struct stat st;

	/* the curl_easy handle is initialized with the klpm handle, so we only need
	...
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "%s: url is %s\n",
		payload->remote_name, payload->fileurl);

	if(payload->max_size) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "%s: maxsize %jd\n",
...
static int curl_retry_next_server(CURLM *curlm, CURL *curl, struct dload_payload *payload)
{
	...
	if((server = payload_next_server(payload)) == NULL) {
		_klpm_log(payload->handle, KUZPKG_LOG_DEBUG,
	...
	MALLOC(payload->fileurl, len, RET_ERR(handle, KUZPKG_ERR_MEMORY, -1));
	...
	if(payload->allow_resume && stat(payload->tempfile_name, &st) == 0) {
		...
		if(ftruncate(fileno(payload->localf), 0)) {
			RET_ERR(handle, KUZPKG_ERR_SYSTEM, -1);
		}
		fseek(payload->localf, 0, SEEK_SET);
	}

	if(handle->dlcb) {
		klpm_download_event_retry_t cb_data;
		cb_data.resume = payload->allow_resume;
		handle->dlcb(handle->dlcb_ctx, payload->remote_name, KUZPKG_DOWNLOAD_RETRY, &cb_data);
	}
 ...
 */
static int curl_check_finished_download(klpm_handle_t *handle, CURLM *curlm, CURLMsg *msg,
		int *active_downloads_num)
{
	...
	ASSERT(curlerr == CURLE_OK, RET_ERR(handle, KUZPKG_ERR_LIBCURL, -1));

	curl_gethost(payload->fileurl, hostname, sizeof(hostname));
	curlerr = msg->data.result;
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "%s: %s returned result %d from transfer\n",
	...
	switch(curlerr) {
		case CURLE_OK:
			...
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "%s: response code %ld\n",
					payload->remote_name, payload->respcode);
			if(payload->respcode >= 400) {
				if(!payload->request_errors_ok) {
					handle->pm_errno = KUZPKG_ERR_RETRIEVE;
		...
		case CURLE_ABORTED_BY_CALLBACK:
			...
			if(dload_interrupted == ABORT_OVER_MAXFILESIZE) {
				...
				payload->unlink_on_fail = 1;
				handle->pm_errno = KUZPKG_ERR_LIBCURL;
				_klpm_log(handle, KUZPKG_LOG_ERROR,
		...
		case CURLE_COULDNT_RESOLVE_HOST:
			handle->pm_errno = KUZPKG_ERR_SERVER_BAD_URL;
			_klpm_log(handle, KUZPKG_LOG_ERROR,
					_("failed retrieving file '%s' from %s : %s\n"),
					payload->remote_name, hostname, payload->error_buffer);
	...
	if(!payload->signature && payload->download_signature && curlerr == CURLE_OK && payload->respcode < 400) {
		...
		char *query;
		const char *dbext = klpm_option_get_dbext(handle);
		const char* realname = payload->destfile_name ? payload->destfile_name : payload->tempfile_name;
		int len;

		STRDUP(_effective_filename, effective_url, GOTO_ERR(handle, KUZPKG_ERR_MEMORY, cleanup));
		...
		len = strlen(url) + 5;
		CALLOC(sig, 1, sizeof(*sig), GOTO_ERR(handle, KUZPKG_ERR_MEMORY, cleanup));
		MALLOC(sig->fileurl, len, FREE(sig); GOTO_ERR(handle, KUZPKG_ERR_MEMORY, cleanup));
		snprintf(sig->fileurl, len, "%s.sig", url);

		int remote_name_len = strlen(payload->remote_name) + 5;
		MALLOC(sig->remote_name, remote_name_len, _klpm_dload_payload_reset(sig);
			FREE(sig); GOTO_ERR(handle, KUZPKG_ERR_MEMORY, cleanup));
		...
		MALLOC(sig->destfile_name, destfile_name_len, _klpm_dload_payload_reset(sig);
				FREE(sig); GOTO_ERR(handle, KUZPKG_ERR_MEMORY, cleanup));
		snprintf(sig->destfile_name, destfile_name_len, "%s.sig", realname);

		int tempfile_name_len = strlen(realname) + 10;
		MALLOC(sig->tempfile_name, tempfile_name_len, _klpm_dload_payload_reset(sig);
				FREE(sig); GOTO_ERR(handle, KUZPKG_ERR_MEMORY, cleanup));
	...
	if(timecond == 1 && bytes_dl == 0) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "%s: file met time condition\n",
			payload->remote_name);
		ret = 1;
	...
	if(remote_size != -1 && bytes_dl != -1 &&
		...
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("%s appears to be truncated: %jd/%jd bytes\n"),
				payload->remote_name, (intmax_t)bytes_dl, (intmax_t)remote_size);
		GOTO_ERR(handle, KUZPKG_ERR_RETRIEVE, cleanup);
	}
...
cleanup:
	...
	if(handle->dlcb) {
		klpm_download_event_completed_t cb_data = {0};
		cb_data.total = bytes_dl;
		cb_data.result = ret;
		handle->dlcb(handle->dlcb_ctx, payload->remote_name, KUZPKG_DOWNLOAD_COMPLETED, &cb_data);
	}
		...
		struct dload_payload *payload)
{
	...
	if(payload->fileurl) {
		ASSERT(!payload->servers, GOTO_ERR(handle, KUZPKG_ERR_WRONG_ARGS, cleanup));
		ASSERT(!payload->filepath, GOTO_ERR(handle, KUZPKG_ERR_WRONG_ARGS, cleanup));
		payload->request_errors_ok = payload->errors_ok;
	} else {
		const char *server = payload_next_server(payload);

		ASSERT(server, GOTO_ERR(handle, KUZPKG_ERR_SERVER_NONE, cleanup));
		ASSERT(payload->filepath, GOTO_ERR(handle, KUZPKG_ERR_WRONG_ARGS, cleanup));

		len = strlen(server) + strlen(payload->filepath) + 2;
		MALLOC(payload->fileurl, len, GOTO_ERR(handle, KUZPKG_ERR_MEMORY, cleanup));
		snprintf(payload->fileurl, len, "%s/%s", server, payload->filepath);
	}

	payload->tempfile_openmode = "wb";
	if(curl_gethost(payload->fileurl, hostname, sizeof(hostname)) != 0) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("url '%s' is invalid\n"), payload->fileurl);
		GOTO_ERR(handle, KUZPKG_ERR_SERVER_BAD_URL, cleanup);
	}
	...
	if(handle->dlcb) {
		klpm_download_event_init_t cb_data = {.optional = payload->errors_ok};
		handle->dlcb(handle->dlcb_ctx, payload->remote_name, KUZPKG_DOWNLOAD_INIT, &cb_data);
	}
 ...
 */
static int curl_download_internal(klpm_handle_t *handle,
		klpm_list_t *payloads /* struct dload_payload */)
{
	...
	size_t payloads_size = klpm_list_count(payloads);
	klpm_list_t *p;

	/* Sort payloads by package size */
	payloads = klpm_list_copy(payloads);
	payloads = klpm_list_msort(payloads, payloads_size, &compare_dload_payload_sizes);
	p = payloads;
	...
	while(active_downloads_num > 0 || p) {
		...
		for(; active_downloads_num < max_streams && p; active_downloads_num++) {
			...
			if(curl_add_payload(handle, curlm, payload) == 0) {
				 ...
				 */
				_klpm_log(handle, KUZPKG_LOG_ERROR, _("failed to setup a download payload for %s\n"), payload->remote_name);
				p = NULL;
				err = -1;
			}
		}
		...
		if(mc != CURLM_OK) {
			_klpm_log(handle, KUZPKG_LOG_ERROR, _("curl returned error %d from transfer\n"), mc);
			p = NULL;
			err = -1;
		}
		while(true) {
			...
			if(msg->msg == CURLMSG_DONE) {
				...
				_klpm_log(handle, KUZPKG_LOG_ERROR, _("curl transfer error: %d\n"), msg->msg);
			}
		}
	}
	int ret = err ? -1 : updated ? 0 : 1;
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "curl_download_internal return code is %d\n", ret);
	klpm_list_free(payloads);
	return ret;
}
 ...
 * Returns 1 if no files were downloaded and all errors were non-fatal
 */
static int curl_download_internal_sandboxed(klpm_handle_t *handle,
		klpm_list_t *payloads /* struct dload_payload */,
		...
		int *childsig)
{
	...
	_klpm_sandbox_callback_context callbacks_ctx;
	...
	if(pid == 0) {
		...
		klpm_option_set_logcb(handle, _klpm_sandbox_cb_log, &callbacks_ctx);
		klpm_option_set_dlcb(handle, _klpm_sandbox_cb_dl, &callbacks_ctx);
		klpm_option_set_fetchcb(handle, NULL, NULL);
		klpm_option_set_eventcb(handle, NULL, NULL);
		klpm_option_set_questioncb(handle, NULL, NULL);
		klpm_option_set_progresscb(handle, NULL, NULL);
		...
		ret = chdir(localpath);
		if(ret != 0) {
			handle->pm_errno = KUZPKG_ERR_NOT_A_DIR;
			_klpm_log(handle, KUZPKG_LOG_ERROR, _("could not chdir to download directory %s\n"), localpath);
			ret = -1;
		} else {
			ret = klpm_sandbox_setup_child(handle, handle->sandboxuser, localpath, true);
			if (ret != 0) {
				_klpm_log(handle, KUZPKG_LOG_ERROR, _("switching to sandbox user '%s' failed!\n"), handle->sandboxuser);
				_Exit(2);
			}
	...
	if(pid != -1)  {
		...
		while(true) {
			_klpm_sandbox_callback_t callback_type;
			...
			if(callback_type == KUZPKG_SANDBOX_CB_DOWNLOAD) {
				if(!_klpm_sandbox_process_cb_download(handle, callbacks_fd[0])) {
					had_error = true;
					break;
				}
			}
			else if(callback_type == KUZPKG_SANDBOX_CB_LOG) {
				if(!_klpm_sandbox_process_cb_log(handle, callbacks_fd[0])) {
					had_error = true;
					break;
				}
			}
		...
		const char *server, const char *localpath)
{
	...
	MALLOC(fileurl, len, RET_ERR(handle, KUZPKG_ERR_MEMORY, -1));
...
static int move_file(const char *filepath, const char *directory)
{
	...
	char *dest = _klpm_get_fullpath(directory, filename, "");
...
static int finalize_download_locations(klpm_list_t *payloads, const char *localpath)
{
	...
	for(p = payloads; p; p = p->next) {
		...
		if(filename) {
			...
			if(ret == -1) {
				if(payload->mtime_existing_file == 0) {
					_klpm_log(payload->handle, KUZPKG_LOG_ERROR, _("could not move %s into %s (%s)\n"),
		...
		if (payload->download_signature) {
			...
			sig_filename = _klpm_get_fullpath("", filename, ".sig");
			ASSERT(sig_filename, RET_ERR(payload->handle, KUZPKG_ERR_MEMORY, -1));
			ret = move_file(sig_filename, localpath);
			free(sig_filename);

			if(ret == -1) {
				sig_filename = _klpm_get_fullpath("", filename, ".sig.part");
				ASSERT(sig_filename, RET_ERR(payload->handle, KUZPKG_ERR_MEMORY, -1));
...
static void prepare_resumable_downloads(klpm_handle_t *handle, klpm_list_t *payloads,
		const char *localpath)
{
	...
	for(p = payloads; p; p = p->next) {
		...
		if(payload->destfile_name) {
			...
			char *dest = _klpm_get_fullpath(localpath, destfilename, "");
		...
		if(!payload->tempfile_name || !_klpm_use_sandbox(handle)) {
			continue;
		}
		const char *filename = mbasename(payload->tempfile_name);
		char *src = _klpm_get_fullpath(localpath, filename, "");
		struct stat st;
...
#else
		RET_ERR(handle, KUZPKG_ERR_EXTERNAL_DOWNLOAD, -1);
#endif
	} else {
		klpm_list_t *p;
		int updated = 0;
		for(p = payloads; p; p = p->next) {
			...
			if(payload->fileurl) {
				...
				if (ret != -1 && payload->download_signature) {
					...
					MALLOC(sig_fileurl, sig_len, RET_ERR(handle, KUZPKG_ERR_MEMORY, -1));
...
download_signature:
	...
	if(_klpm_use_sandbox(handle)) {
		...
		_klpm_remove_temporary_download_dir(temporary_localpath);
	}
	...
	if(finalize_ret != 0 && ret == 0) {
		RET_ERR(handle, KUZPKG_ERR_RETRIEVE, -1);
	}
...
int SYMEXPORT klpm_fetch_pkgurl(klpm_handle_t *handle, const klpm_list_t *urls,
	  klpm_list_t **fetched)
{
	klpm_siglevel_t siglevel = klpm_option_get_remote_file_siglevel(handle);
	const char *cachedir;
	...
	ASSERT(*fetched == NULL, RET_ERR(handle, KUZPKG_ERR_WRONG_ARGS, -1));
	...
	for(i = urls; i; i = i->next) {
		...
		if(urlbase) {
			...
			filepath = _klpm_filecache_find(handle, urlbase);

			if(filepath && (siglevel & KUZPKG_SIG_PACKAGE)) {
				char *sig_filename = _klpm_get_fullpath("", urlbase, ".sig");

				/* if there's no .sig file then forget about the pkg file and go for download */
				if(!_klpm_filecache_exists(handle, sig_filename)) {
					free(filepath);
					filepath = NULL;
				}
		...
		if(filepath) {
			...
			klpm_list_append(fetched, filepath);
		} else {
			struct dload_payload *payload = NULL;
			char *c;

			ASSERT(url, GOTO_ERR(handle, KUZPKG_ERR_WRONG_ARGS, err));
			CALLOC(payload, 1, sizeof(*payload), GOTO_ERR(handle, KUZPKG_ERR_MEMORY, err));
			STRDUP(payload->fileurl, url, FREE(payload); GOTO_ERR(handle, KUZPKG_ERR_MEMORY, err));

			STRDUP(payload->remote_name, get_filename(payload->fileurl),
				GOTO_ERR(handle, KUZPKG_ERR_MEMORY, err));
	...
	if(payloads) {
		...
		event.pkg_retrieve.num = klpm_list_count(payloads);
		event.pkg_retrieve.total_size = 0;
		EVENT(handle, &event);
		if(_klpm_download(handle, payloads, cachedir, temporary_cachedir) == -1) {
			_klpm_log(handle, KUZPKG_LOG_WARNING, _("failed to retrieve some files\n"));
			event.type = KUZPKG_EVENT_PKG_RETRIEVE_FAILED;
			EVENT(handle, &event);
			GOTO_ERR(handle, KUZPKG_ERR_RETRIEVE, err);
		} else {
		...
		for(i = payloads; i; i = i->next) {
			...
			if(payload->destfile_name) {
				...
				filepath = _klpm_filecache_find(handle, filename);
			} else {
				const char *filename = mbasename(payload->tempfile_name);
				filepath = _klpm_filecache_find(handle, filename);
			}
			if(filepath) {
				...
				_klpm_log(handle, KUZPKG_LOG_WARNING, _("download completed successfully but no file in the cache\n"));
				GOTO_ERR(handle, KUZPKG_ERR_RETRIEVE, err);
			}
		}
