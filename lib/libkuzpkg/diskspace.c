...
{
	const klpm_mountpoint_t *mp1 = p1;
	const klpm_mountpoint_t *mp2 = p2;
	/* the negation will sort all mountpoints before their parent */
	return -strcmp(mp1->mount_dir, mp2->mount_dir);
}

static void mount_point_list_free(klpm_list_t *mount_points)
{
	klpm_list_t *i;

	for(i = mount_points; i; i = i->next) {
		klpm_mountpoint_t *data = i->data;
		FREE(data->mount_dir);
	}
	FREELIST(mount_points);
}

static int mount_point_load_fsinfo(klpm_handle_t *handle, klpm_mountpoint_t *mountpoint)
{
	...
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "loading fsinfo for %s\n", mountpoint->mount_dir);
...
static klpm_list_t *mount_point_list(klpm_handle_t *handle)
{
	klpm_list_t *mount_points = NULL, *ptr;
	klpm_mountpoint_t *mp;
	...
	while((mnt = getmntent(fp))) {
		...
		CALLOC(mp, 1, sizeof(klpm_mountpoint_t), RET_ERR(handle, KUZPKG_ERR_MEMORY, NULL));
		STRDUP(mp->mount_dir, mnt->mnt_dir, free(mp); RET_ERR(handle, KUZPKG_ERR_MEMORY, NULL));
		mp->mount_dir_len = strlen(mp->mount_dir);

		mount_points = klpm_list_add(mount_points, mp);
	}
	...
	while((ret = getmntent(fp, &mnt)) == 0) {
		...
		CALLOC(mp, 1, sizeof(klpm_mountpoint_t), RET_ERR(handle, KUZPKG_ERR_MEMORY, NULL));
		STRDUP(mp->mount_dir, mnt->mnt_mountp,  free(mp); RET_ERR(handle, KUZPKG_ERR_MEMORY, NULL));
		mp->mount_dir_len = strlen(mp->mount_dir);
	...
	for(; entries-- > 0; fsp++) {
		...
		CALLOC(mp, 1, sizeof(klpm_mountpoint_t), RET_ERR(handle, KUZPKG_ERR_MEMORY, NULL));
		STRDUP(mp->mount_dir, fsp->f_mntonname, free(mp); RET_ERR(handle, KUZPKG_ERR_MEMORY, NULL));
		mp->mount_dir_len = strlen(mp->mount_dir);
	...
	mount_points = klpm_list_msort(mount_points, klpm_list_count(mount_points),
			mount_point_cmp);
	for(ptr = mount_points; ptr != NULL; ptr = ptr->next) {
		mp = ptr->data;
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "discovered mountpoint: %s\n", mp->mount_dir);
	}
	return mount_points;
}

static klpm_mountpoint_t *match_mount_point(const klpm_list_t *mount_points,
		const char *real_path)
{
	const klpm_list_t *mp;

	for(mp = mount_points; mp != NULL; mp = mp->next) {
		klpm_mountpoint_t *data = mp->data;
...
static int calculate_removed_size(klpm_handle_t *handle,
		const klpm_list_t *mount_points, klpm_pkg_t *pkg)
{
	size_t i;
	klpm_filelist_t *filelist = klpm_pkg_get_files(pkg);
	...
	for(i = 0; i < filelist->count; i++) {
		const klpm_file_t *file = filelist->files + i;
		klpm_mountpoint_t *mp;
		struct stat st;
		char path[PATH_MAX];
		...
		if(llstat(path, &st) == -1) {
			if(klpm_option_match_noextract(handle, filename)) {
				_klpm_log(handle, KUZPKG_LOG_WARNING,
...
static int calculate_installed_size(klpm_handle_t *handle,
		const klpm_list_t *mount_points, klpm_pkg_t *pkg)
{
	size_t i;
	klpm_filelist_t *filelist = klpm_pkg_get_files(pkg);
...
static int check_mountpoint(klpm_handle_t *handle, klpm_mountpoint_t *mp)
{
...
int _klpm_check_downloadspace(klpm_handle_t *handle, const char *cachedir,
		size_t num_files, const off_t *file_sizes)
{
	klpm_list_t *mount_points;
	klpm_mountpoint_t *cachedir_mp;
	...
	if(mount_points == NULL) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("could not determine filesystem mount points\n"));
		return -1;
	}

	cachedir_mp = match_mount_point(mount_points, cachedir);
	if(cachedir_mp == NULL) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("could not determine cachedir mount point %s\n"),
				cachedir);
		error = 1;
		goto finish;
	}
	 ...
	 * _klpm_filecache_setup will never give us a non-writable directory */
...
finish:
	...
	if(error) {
		RET_ERR(handle, KUZPKG_ERR_DISK_SPACE, -1);
	}

	return 0;
}

int _klpm_check_diskspace(klpm_handle_t *handle)
{
	klpm_list_t *mount_points, *i;
	klpm_mountpoint_t *root_mp;
	size_t replaces = 0, current = 0, numtargs;
	int error = 0;
	klpm_list_t *targ;
	klpm_trans_t *trans = handle->trans;

	numtargs = klpm_list_count(trans->add);
	mount_points = mount_point_list(handle);
	if(mount_points == NULL) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("could not determine filesystem mount points\n"));
		return -1;
	...
	if(root_mp == NULL) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("could not determine root mount point %s\n"),
				handle->root);
		error = 1;
		goto finish;
	}

	replaces = klpm_list_count(trans->remove);
	if(replaces) {
		numtargs += replaces;
		for(targ = trans->remove; targ; targ = targ->next, current++) {
			klpm_pkg_t *local_pkg;
			int percent = (current * 100) / numtargs;
			PROGRESS(handle, KUZPKG_PROGRESS_DISKSPACE_START, "", percent,
					numtargs, current);
	...
	for(targ = trans->add; targ; targ = targ->next, current++) {
		...
		local_pkg = _klpm_db_get_pkgfromcache(handle->db_local, pkg->name);
		if(local_pkg) {
			calculate_removed_size(handle, mount_points, local_pkg);
		}
	...
	PROGRESS(handle, KUZPKG_PROGRESS_DISKSPACE_START, "", 100,
			numtargs, current);

	for(i = mount_points; i; i = i->next) {
		klpm_mountpoint_t *data = i->data;
		if(data->used && data->read_only) {
			_klpm_log(handle, KUZPKG_LOG_ERROR, _("Partition %s is mounted read only\n"),
					data->mount_dir);
			error = 1;
