		...
		klpm_filelist_t *filesB)
{
	...
	while(ctrA < filesA->count) {
		ret = klpm_list_add(ret, filesA->files[ctrA].name);
		ctrA++;
	}
...
static int _klpm_filelist_pathcmp(const char *p1, const char *p2)
{
	while(*p1 && *p1 == *p2) {
		p1++;
		p2++;
		...
		klpm_filelist_t *filesB)
{
	klpm_list_t *ret = NULL;
	size_t ctrA = 0, ctrB = 0;
	klpm_file_t *arrA = filesA->files, *arrB = filesB->files;

	while(ctrA < filesA->count && ctrB < filesB->count) {
		const char *strA = arrA[ctrA].name, *strB = arrB[ctrB].name;
		int cmp = _klpm_filelist_pathcmp(strA, strB);
		if(cmp < 0) {
			ctrA++;
		} else if(cmp > 0) {
			ctrB++;
			...
			if(strA[strlen(strA) - 1] != '/' || strB[strlen(strB) - 1] != '/') {
				ret = klpm_list_add(ret, arrA[ctrA].name);
			}
			ctrA++;
			ctrB++;
		}
 ...
 */
static int _klpm_files_cmp(const void *f1, const void *f2)
{
	const klpm_file_t *file1 = f1;
	const klpm_file_t *file2 = f2;
	return strcmp(file1->name, file2->name);
}

klpm_file_t SYMEXPORT *klpm_filelist_contains(const klpm_filelist_t *filelist,
		const char *path)
{
	klpm_file_t key;
	...
	return bsearch(&key, filelist->files, filelist->count,
			sizeof(klpm_file_t), _klpm_files_cmp);
}
