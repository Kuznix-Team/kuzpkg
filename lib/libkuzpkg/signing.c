...
int SYMEXPORT klpm_decode_signature(const char *base64_data,
...
static void sigsum_test_bit(gpgme_sigsum_t sigsum, klpm_list_t **summary,
		gpgme_sigsum_t bit, const char *value)
{
	if(sigsum & bit) {
		*summary = klpm_list_add(*summary, (void *)value);
	}
}
 ...
 */
static klpm_list_t *list_sigsum(gpgme_sigsum_t sigsum)
{
	klpm_list_t *summary = NULL;
	...
	/* Fallback case */
	if(!sigsum) {
		summary = klpm_list_add(summary, (void *)"(empty)");
	}
	return summary;
}
 ...
 */
static int init_gpgme(klpm_handle_t *handle)
{
	static int init = 0;
	...
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "GPGME version: %s\n", version);
	...
	CHECK_ERR();
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "GPGME engine info: file=%s, home=%s\n",
...
gpg_error:
	_klpm_log(handle, KUZPKG_LOG_ERROR, _("GPGME error: %s\n"), gpgme_strerror(gpg_err));
	RET_ERR(handle, KUZPKG_ERR_GPGME, -1);
}
...
int _klpm_key_in_keychain(klpm_handle_t *handle, const char *fpr)
{
	...
	if(klpm_list_find_str(handle->known_keys, fpr)) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "key %s found in cache\n", fpr);
		return 1;
	}
	...
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "looking up key %s locally\n", fpr);

	gpg_err = gpgme_get_key(ctx, fpr, &key, 0);
	if(gpg_err_code(gpg_err) == GPG_ERR_EOF) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "key lookup failed, unknown key\n");
		ret = 0;
	} else if(gpg_err_code(gpg_err) == GPG_ERR_NO_ERROR) {
		if(key->expired) {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "key lookup success, but key is expired\n");
			ret = 0;
		} else {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "key lookup success, key exists\n");
			handle->known_keys = klpm_list_add(handle->known_keys, strdup(fpr));
			ret = 1;
		}
	} else {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "gpg error: %s\n", gpgme_strerror(gpg_err));
	}
	gpgme_key_unref(key);
 ...
 * @return 0 on success, -1 on error
 */
static int key_import_wkd(klpm_handle_t *handle, const char *email, const char *fpr)
{
	gpgme_error_t gpg_err;
	gpgme_ctx_t ctx = {0};
	...
	_klpm_log(handle, KUZPKG_LOG_DEBUG, _("looking up key %s using WKD\n"), email);
	gpg_err = gpgme_get_key(ctx, email, &key, 0);
	if(gpg_err_code(gpg_err) == GPG_ERR_NO_ERROR) {
		/* check if correct key was imported via WKD */
		if(fpr && _klpm_key_in_keychain(handle, fpr)) {
			ret = 0;
		} else {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "key lookup failed: WKD imported wrong fingerprint or key expired\n");
		}
	}
	gpgme_key_unref(key);
...
gpg_error:
	if(ret != 0) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, _("gpg error: %s\n"), gpgme_strerror(gpg_err));
	}
	gpgme_release(ctx);
	return ret;
}
 ...
 */
static int key_search_keyserver(klpm_handle_t *handle, const char *fpr,
		klpm_pgpkey_t *pgpkey)
{
	gpgme_error_t gpg_err;
	gpgme_ctx_t ctx = {0};
	gpgme_keylist_mode_t mode;
	...
	fpr_len = strlen(fpr);
	MALLOC(full_fpr, fpr_len + 3, RET_ERR(handle, KUZPKG_ERR_MEMORY, -1));
	...
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "looking up key %s remotely\n", fpr);
 ...
 * @return 0 on success, -1 on error
 */
static int key_import_keyserver(klpm_handle_t *handle, klpm_pgpkey_t *key)
{
	gpgme_error_t gpg_err;
	gpgme_ctx_t ctx = {0};
	gpgme_key_t keys[2];
	...
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "importing key\n");
	...
	if(result->considered != 1 || !result->imports) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "could not import key, 0 results\n");
		ret = -1;
...
int _klpm_key_import(klpm_handle_t *handle, const char *uid, const char *fpr)
{
	...
	klpm_question_import_key_t question = {
				.type = KUZPKG_QUESTION_IMPORT_KEY,
				.import = 0,
				.uid = uid,
				.fingerprint = fpr
			};
...
int _klpm_gpgme_checksig(klpm_handle_t *handle, const char *path,
		const char *base64_sig, klpm_siglist_t *siglist)
{
	int ret = -1, sigcount;
	...
	if(!path || _klpm_access(handle, NULL, path, R_OK) != 0) {
		RET_ERR(handle, KUZPKG_ERR_NOT_A_FILE, -1);
	}
	...
	if(!base64_sig) {
		sigpath = _klpm_sigpath(handle, path);
		if(_klpm_access(handle, NULL, sigpath, R_OK) != 0
				|| (sigfile = fopen(sigpath, "rb")) == NULL) {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "sig path %s could not be opened\n",
					sigpath);
			GOTO_ERR(handle, KUZPKG_ERR_SIG_MISSING, error);
		}
	}
	...
	file = fopen(path, "rb");
	if(file == NULL) {
		GOTO_ERR(handle, KUZPKG_ERR_NOT_A_FILE, error);
	}
	...
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "checking signature for %s\n", path);
	...
	if(base64_sig) {
		...
		size_t data_len;
		int decode_ret = klpm_decode_signature(base64_sig,
				&decoded_sigdata, &data_len);
		if(decode_ret) {
			GOTO_ERR(handle, KUZPKG_ERR_SIG_INVALID, error);
		}
	...
	if(!verify_result || !verify_result->signatures) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "no signatures returned\n");
		GOTO_ERR(handle, KUZPKG_ERR_SIG_MISSING, gpg_error);
	}
	for(gpgsig = verify_result->signatures, sigcount = 0;
			gpgsig; gpgsig = gpgsig->next, sigcount++);
	_klpm_log(handle, KUZPKG_LOG_DEBUG, "%d signatures returned\n", sigcount);

	CALLOC(siglist->results, sigcount, sizeof(klpm_sigresult_t),
			GOTO_ERR(handle, KUZPKG_ERR_MEMORY, gpg_error));
	siglist->count = sigcount;

	for(gpgsig = verify_result->signatures, sigcount = 0; gpgsig;
			gpgsig = gpgsig->next, sigcount++) {
		klpm_list_t *summary_list, *summary;
		klpm_sigstatus_t status;
		klpm_sigvalidity_t validity;
		gpgme_key_t key;
		klpm_sigresult_t *result;

		_klpm_log(handle, KUZPKG_LOG_DEBUG, "fingerprint: %s\n", gpgsig->fpr);
		summary_list = list_sigsum(gpgsig->summary);
		for(summary = summary_list; summary; summary = summary->next) {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "summary: %s\n", (const char *)summary->data);
		}
		klpm_list_free(summary_list);
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "status: %s\n", gpgme_strerror(gpgsig->status));
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "timestamp: %lu\n", gpgsig->timestamp);
		...
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "exp_timestamp: %lu\n", gpgsig->exp_timestamp);
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "validity: %s; reason: %s\n",
		...
		switch(gpg_err_code(gpgsig->status)) {
			...
			case GPG_ERR_NO_ERROR:
				status = KUZPKG_SIGSTATUS_VALID;
				break;
			case GPG_ERR_KEY_EXPIRED:
				status = KUZPKG_SIGSTATUS_KEY_EXPIRED;
				break;
			/* bad cases */
			case GPG_ERR_SIG_EXPIRED:
				status = KUZPKG_SIGSTATUS_SIG_EXPIRED;
				break;
			case GPG_ERR_NO_PUBKEY:
				status = KUZPKG_SIGSTATUS_KEY_UNKNOWN;
				break;
			case GPG_ERR_BAD_SIGNATURE:
			default:
				status = KUZPKG_SIGSTATUS_INVALID;
				break;
		}
		/* special case: key disabled is not returned in above status code */
		if(result->key.data && key->disabled) {
			status = KUZPKG_SIGSTATUS_KEY_DISABLED;
		}

		switch(gpgsig->validity) {
			case GPGME_VALIDITY_ULTIMATE:
			case GPGME_VALIDITY_FULL:
				validity = KUZPKG_SIGVALIDITY_FULL;
				break;
			case GPGME_VALIDITY_MARGINAL:
				validity = KUZPKG_SIGVALIDITY_MARGINAL;
				break;
			case GPGME_VALIDITY_NEVER:
				validity = KUZPKG_SIGVALIDITY_NEVER;
				break;
			case GPGME_VALIDITY_UNKNOWN:
			case GPGME_VALIDITY_UNDEFINED:
			default:
				validity = KUZPKG_SIGVALIDITY_UNKNOWN;
				break;
		}
...
#else /* HAVE_LIBGPGME */
int _klpm_key_in_keychain(klpm_handle_t *handle, const char UNUSED *fpr)
{
	handle->pm_errno = KUZPKG_ERR_MISSING_CAPABILITY_SIGNATURES;
	return -1;
}

int _klpm_key_import(klpm_handle_t *handle, const char UNUSED *uid,
...
int _klpm_gpgme_checksig(klpm_handle_t *handle, const char UNUSED *path,
		const char UNUSED *base64_sig, klpm_siglist_t *siglist)
{
	siglist->count = 0;
	handle->pm_errno = KUZPKG_ERR_MISSING_CAPABILITY_SIGNATURES;
	return -1;
...
char *_klpm_sigpath(klpm_handle_t *handle, const char *path)
{
	...
	len = strlen(path) + 5;
	CALLOC(sigpath, len, sizeof(char), RET_ERR(handle, KUZPKG_ERR_MEMORY, NULL));
 ...
 * This wraps #_klpm_gpgme_checksig in a slightly friendlier manner to simplify
		...
		klpm_siglist_t **sigdata)
{
	...
	CALLOC(siglist, 1, sizeof(klpm_siglist_t),
			RET_ERR(handle, KUZPKG_ERR_MEMORY, -1));

	ret = _klpm_gpgme_checksig(handle, path, base64_sig, siglist);
	if(ret && handle->pm_errno == KUZPKG_ERR_SIG_MISSING) {
		if(optional) {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "missing optional signature\n");
			handle->pm_errno = KUZPKG_ERR_OK;
			ret = 0;
		} else {
			_klpm_log(handle, KUZPKG_LOG_DEBUG, "missing required signature\n");
			/* ret will already be -1 */
		}
	} else if(ret) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, "signature check failed\n");
		/* ret will already be -1 */
	} else {
		size_t num;
		for(num = 0; !ret && num < siglist->count; num++) {
			switch(siglist->results[num].status) {
				...
				case KUZPKG_SIGSTATUS_KEY_EXPIRED:
					_klpm_log(handle, KUZPKG_LOG_DEBUG, "signature is valid\n");
					switch(siglist->results[num].validity) {
						case KUZPKG_SIGVALIDITY_FULL:
							_klpm_log(handle, KUZPKG_LOG_DEBUG, "signature is fully trusted\n");
							break;
						case KUZPKG_SIGVALIDITY_MARGINAL:
							_klpm_log(handle, KUZPKG_LOG_DEBUG, "signature is marginal trust\n");
							if(!marginal) {
								ret = -1;
							}
							break;
						case KUZPKG_SIGVALIDITY_UNKNOWN:
							_klpm_log(handle, KUZPKG_LOG_DEBUG, "signature is unknown trust\n");
							if(!unknown) {
								ret = -1;
							}
							break;
						case KUZPKG_SIGVALIDITY_NEVER:
							_klpm_log(handle, KUZPKG_LOG_DEBUG, "signature should never be trusted\n");
							ret = -1;
							break;
					}
					break;
				...
				case KUZPKG_SIGSTATUS_INVALID:
					_klpm_log(handle, KUZPKG_LOG_DEBUG, "signature is not valid\n");
					ret = -1;
					break;
			}
		}
	...
	for(i = 0; i < siglist->count; i++) {
		...
		switch(result->status) {
					...
					case KUZPKG_SIGVALIDITY_MARGINAL:
						if(!marginal) {
							_klpm_log(handle, KUZPKG_LOG_ERROR,
									...
									identifier, name);
							/* QUESTION(handle, KUZPKG_QUESTION_EDIT_KEY_TRUST, &result->key, NULL, NULL, &answer); */
						}
						break;
					case KUZPKG_SIGVALIDITY_UNKNOWN:
						if(!unknown) {
			...
			case KUZPKG_SIGSTATUS_KEY_EXPIRED:
				...
				if(_klpm_key_import(handle, result->key.uid, result->key.fingerprint) == 0) {
					retry = 1;
				}
			...
			case KUZPKG_SIGSTATUS_KEY_UNKNOWN:
				...
				if(_klpm_key_in_keychain(handle, result->key.fingerprint) == 1) {
					break;
				}
				_klpm_log(handle, KUZPKG_LOG_ERROR,
...
int SYMEXPORT klpm_pkg_check_pgp_signature(klpm_pkg_t *pkg,
		klpm_siglist_t *siglist)
{
	ASSERT(pkg != NULL, return -1);
	ASSERT(siglist != NULL, RET_ERR(pkg->handle, KUZPKG_ERR_WRONG_ARGS, -1));
	pkg->handle->pm_errno = KUZPKG_ERR_OK;

	return _klpm_gpgme_checksig(pkg->handle, pkg->filename,
			pkg->base64_sig, siglist);
}

int SYMEXPORT klpm_db_check_pgp_signature(klpm_db_t *db,
		klpm_siglist_t *siglist)
{
	ASSERT(db != NULL, return -1);
	ASSERT(siglist != NULL, RET_ERR(db->handle, KUZPKG_ERR_WRONG_ARGS, -1));
	db->handle->pm_errno = KUZPKG_ERR_OK;

	return _klpm_gpgme_checksig(db->handle, _klpm_db_path(db), NULL, siglist);
}

int SYMEXPORT klpm_siglist_cleanup(klpm_siglist_t *siglist)
{
	ASSERT(siglist != NULL, return -1);
	size_t num;
	for(num = 0; num < siglist->count; num++) {
		klpm_sigresult_t *result = siglist->results + num;
		if(result->key.data) {
#ifdef HAVE_LIBGPGME
...
static size_t length_check(size_t length, size_t position, size_t a,
		klpm_handle_t *handle, const char *identifier)
{
...
static int parse_subpacket(klpm_handle_t *handle, const char *identifier,
		const unsigned char *sig, const size_t len, const size_t pos,
		const size_t plen, klpm_list_t **keys)
{
		size_t slen;
		size_t spos = pos;
		...
		while(spos < pos + plen) {
			...
			if(sig[spos] == 16) {
				...
				*keys = klpm_list_add(*keys, strdup(key));
				break;
			}
...
int SYMEXPORT klpm_extract_keyid(klpm_handle_t *handle, const char *identifier,
