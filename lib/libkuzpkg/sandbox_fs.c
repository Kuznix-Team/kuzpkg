...
bool _klpm_sandbox_fs_restrict_writes_to(klpm_handle_t *handle, const char *path)
{
	...
	if(abi < 0) {
		...
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("restricting filesystem access failed because Landlock is not supported by the kernel!\n"));
		return false;
	}
#ifdef LANDLOCK_ACCESS_FS_REFER
	if(abi < 2) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, _("Landlock ABI < 2, LANDLOCK_ACCESS_FS_REFER is not supported\n"));
		ruleset_attr.handled_access_fs &= ~LANDLOCK_ACCESS_FS_REFER;
	}
#endif /* LANDLOCK_ACCESS_FS_REFER */
#ifdef LANDLOCK_ACCESS_FS_TRUNCATE
	if(abi < 3) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, _("Landlock ABI < 3, LANDLOCK_ACCESS_FS_TRUNCATE is not supported\n"));
	...
	if(ruleset_fd < 0) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("restricting filesystem access failed because the Landlock ruleset could not be created: %s\n"), strerror(errno));
		return false;
	}
	...
	if(path_beneath.parent_fd == -1) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("opening the root filesystem to make it read-only via Landlock failed: %s\n"), strerror(errno));
		return false;
	}
	...
	if(landlock_add_rule(ruleset_fd, LANDLOCK_RULE_PATH_BENEATH, &path_beneath, 0) != 0) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("restricting filesystem access failed because the Landlock rule for / could not be added: %s\n"), strerror(errno));
	...
	if(path_beneath.parent_fd == -1) {
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("opening the download directory to make it writable via Landlock failed: %s\n"), strerror(errno));
		return false;
	}
	...
	if(landlock_add_rule(ruleset_fd, LANDLOCK_RULE_PATH_BENEATH, &path_beneath, 0) == 0) {
		if(landlock_restrict_self(ruleset_fd, 0)) {
			result = errno;
			_klpm_log(handle, KUZPKG_LOG_ERROR, _("restricting filesystem access failed because the Landlock ruleset could not be applied: %s\n"), strerror(result));
		}
	} else {
		result = errno;
		_klpm_log(handle, KUZPKG_LOG_ERROR, _("restricting filesystem access failed because the Landlock rule for the temporary download directory could not be added: %s\n"), strerror(result));
	}

	close(path_beneath.parent_fd);
	close(ruleset_fd);
	if(result == 0) {
		_klpm_log(handle, KUZPKG_LOG_DEBUG, _("filesystem access has been restricted to %s, Landlock ABI is %d\n"), path, abi);
		return true;
        }
	return false;
