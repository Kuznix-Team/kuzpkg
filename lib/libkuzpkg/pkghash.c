...
{
	klpm_pkghash_t *hash = NULL;
	unsigned int i, loopsize;

	CALLOC(hash, 1, sizeof(klpm_pkghash_t), return NULL);
	...
	CALLOC(hash->hash_table, hash->buckets, sizeof(klpm_list_t *), \
...
static klpm_pkghash_t *rehash(klpm_pkghash_t *oldhash)
{
	klpm_pkghash_t *newhash;
	...
	newhash = _klpm_pkghash_create(newsize);
	if(newhash == NULL) {
		return NULL;
	}
	...
	for(i = 0; i < oldhash->buckets; i++) {
		if(oldhash->hash_table[i] != NULL) {
			klpm_pkg_t *package = oldhash->hash_table[i]->data;
	...
	_klpm_pkghash_free(oldhash);

	return newhash;
}

static klpm_pkghash_t *pkghash_add_pkg(klpm_pkghash_t **hashref, klpm_pkg_t *pkg,
		int sorted)
{
	klpm_list_t *ptr;
	unsigned int position;
	...
	MALLOC(ptr, sizeof(klpm_list_t), return NULL);
	...
	if(!sorted) {
		hash->list = klpm_list_join(hash->list, ptr);
	} else {
		hash->list = klpm_list_mmerge(hash->list, ptr, _klpm_pkg_cmp);
	}
...
static unsigned int move_one_entry(klpm_pkghash_t *hash,
		unsigned int start, unsigned int end)
{
	...
	while(end != start) {
		klpm_list_t *i = hash->hash_table[end];
		klpm_pkg_t *info = i->data;
 ...
 */
klpm_pkghash_t *_klpm_pkghash_remove(klpm_pkghash_t *hash, klpm_pkg_t *pkg,
		klpm_pkg_t **data)
{
	klpm_list_t *i;
	unsigned int position;
	...
	while((i = hash->hash_table[position]) != NULL) {
		...
		if(info->name_hash == pkg->name_hash &&
			...
			hash->list = klpm_list_remove_item(hash->list, i);
			if(data) {
				*data = info;
			}
