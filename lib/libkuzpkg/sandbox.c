...
int SYMEXPORT klpm_sandbox_setup_child(klpm_handle_t *handle, const char* sandboxuser, const char* sandbox_path, bool restrict_syscalls)
{
	...
	if(sandbox_path != NULL && !handle->disable_sandbox_filesystem) {
		ASSERT(_klpm_sandbox_fs_restrict_writes_to(handle, sandbox_path), return -1);
	}
	...
	if(restrict_syscalls && !handle->disable_sandbox_syscalls) {
		ASSERT(_klpm_sandbox_syscalls_filter(handle), return -1);
	}
...
{
	_klpm_sandbox_callback_t type = KUZPKG_SANDBOX_CB_LOG;
	_klpm_sandbox_callback_context *context = ctx;
	char *string = NULL;
	int string_size = 0;
...
{
	_klpm_sandbox_callback_t type = KUZPKG_SANDBOX_CB_DOWNLOAD;
	_klpm_sandbox_callback_context *context = ctx;
	size_t filename_len;
	...
	ASSERT(event == KUZPKG_DOWNLOAD_INIT || event == KUZPKG_DOWNLOAD_PROGRESS || event == KUZPKG_DOWNLOAD_RETRY || event == KUZPKG_DOWNLOAD_COMPLETED, return);
	...
	switch(event) {
		case KUZPKG_DOWNLOAD_INIT:
			write_to_pipe(context->callback_pipe, data, sizeof(klpm_download_event_init_t));
			break;
		case KUZPKG_DOWNLOAD_PROGRESS:
			write_to_pipe(context->callback_pipe, data, sizeof(klpm_download_event_progress_t));
			break;
		case KUZPKG_DOWNLOAD_RETRY:
			write_to_pipe(context->callback_pipe, data, sizeof(klpm_download_event_retry_t));
			break;
		case KUZPKG_DOWNLOAD_COMPLETED:
			write_to_pipe(context->callback_pipe, data, sizeof(klpm_download_event_completed_t));
			break;
	}
...
bool _klpm_sandbox_process_cb_log(klpm_handle_t *handle, int callback_pipe) {
	...
	_klpm_log(handle, level, "%s", string);
	FREE(string);
	return true;
}

bool _klpm_sandbox_process_cb_download(klpm_handle_t *handle, int callback_pipe) {
	klpm_download_event_type_t type;
	char *filename = NULL;
	size_t filename_size, cb_data_size;
	union {
		klpm_download_event_init_t init;
		klpm_download_event_progress_t progress;
		klpm_download_event_retry_t retry;
		klpm_download_event_completed_t completed;
	} cb_data;
