...
class pmtest(object):
    ...
    def generate(self, kuzpkg):
        ...
        ldconfig = os.path.basename(kuzpkg["ldconfig"])
        ldconfigdir = os.path.join(self.root, os.path.dirname(kuzpkg["ldconfig"][1:]))
        shell = kuzpkg["scriptlet-shell"][1:]
        ...
        if self.db["local"].pkgs and self.dbver >= 9:
            ...
            util.mkfile(path, "KUZPKG_DB_VERSION", str(self.dbver))
    ...
    def add_hook(self, name, content):
        if not name.endswith(".hook"):
            name = name + ".hook"
        path = os.path.join("etc/kuzpkg.d/hooks/", name)
    ...
    def run(self, kuzpkg):
        if os.path.isfile(util.PM_LOCK):
            tap.bail("\tERROR: another kuzpkg session is on-going -- skipping")
            return

        tap.diag("==> Running test")
        vprint("\tkuzpkg %s" % self.args)
        ...
        prog = kuzpkg["bin"]
        if not prog:
            prog = util.which(self.cmd[0], kuzpkg["bindir"])
        ...
        if kuzpkg["manual-confirm"]:
            cmd.append("--confirm")
        if kuzpkg["debug"]:
            cmd.append("--debug=%s" % kuzpkg["debug"])
        cmd.extend(shlex.split(self.args))

        if not (kuzpkg["gdb"] or kuzpkg["nolog"]):
        ...
        # Change to the tmp dir before running kuzpkg, so that local package
        ...
        if os.path.isfile(os.path.join(self.root, util.TMPDIR, "core")):
            tap.diag("\tERROR: kuzpkg dumped a core file")
