#! /usr/bin/python3
#
#  pactest : run automated testing on the kuzpkg binary
#
...
def create_parser():
    ...
    description = "Runs automated tests on the kuzpkg binary. Tests are " \
    ...
    parser.add_option("-d", "--debug", type = "int",
                      ...
                      help = "set debug level for kuzpkg")
    parser.add_option("-p", "--kuzpkg", type = "string",
                      dest = "bin", default = None,
                      help = "specify location of the kuzpkg binary")
...
if __name__ == "__main__":
    ...
    env.kuzpkg["debug"] = opts.debug
    env.kuzpkg["bin"] = opts.bin
    env.kuzpkg["bindir"] = opts.bindir
    env.kuzpkg["nolog"] = opts.nolog
    env.kuzpkg["gdb"] = opts.gdb
    env.kuzpkg["valgrind"] = opts.valgrind
    env.kuzpkg["manual-confirm"] = opts.manualconfirm
    env.kuzpkg["scriptlet-shell"] = opts.scriptletshell
    env.kuzpkg["ldconfig"] = opts.ldconfig
    ...
    if not opts.keeproot:
        ...
        tap.diag("kuzpkg testing root saved: %s" % root_path)
