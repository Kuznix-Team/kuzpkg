...
This document is here to guide you in helping translate kuzpkg messages,
libkuzpkg messages, and the manual pages for the entire kuzpkg package.

We are currently using https://app.transifex.com/[Transifex] as the translation
platform for kuzpkg and libkuzpkg. You will need to sign up for an account there
and then register with a translation team on the
https://explore.transifex.com/toofishes/archlinux-kuzpkg/[kuzpkg project page].
...
There are two separate message catalogs in kuzpkg: one for the back-end
(libkuzpkg) and one for the front-end (kuzpkg and scripts). These correspond to
the `lib/libkuzpkg/po` and `po` directories in the kuzpkg source, respectively.
...
resources (See `tx status` output for a quick overview).
	...
	tx pull -f
	poedit po/<mylang>.po
	poedit lib/libkuzpkg/po/<mylang>.po
	tx push -t -l <mylang>

Or to just push one of the two available resources:

	tx push -r archlinux-kuzpkg.kuzpkg-pot -t -l <mylang>
	tx push -r archlinux-kuzpkg.libkuzpkg-pot -t -l <mylang>
...
and an email will be sent to the mailto:kuzpkg-dev@lists.archlinux.org[kuzpkg-dev]
...
Transifex project page.  Each language will have three files available (libkuzpkg,
kuzpkg and kuzpkg-scripts). Translators interested in helping are encouraged to use the
...
If you have more advanced needs you will have to get a copy of the kuzpkg
repository.

	git clone https://gitlab.archlinux.org/kuzpkg/kuzpkg.git
...
the source code by running:
	...
	meson compile -C build kuzpkg-scripts-pot kuzpkg-pot libkuzpkg-pot
	
We need to first update the main message catalog file. Navigate into either
the `lib/libkuzpkg/po`, `scripts/po` or `src/kuzpkg/po` directory depending
...
with `libkuzpkg.pot`, `kuzpkg.pot` or `kuzpkg-scripts.pot`):
...
You will have to do the following steps in both the `lib/libkuzpkg/po/` and `po/`
...
substituting `libkuzpkg.pot` or `kuzpkg.pot` for potfile depending on which
...
you may ignore the rest), replacing `<lang_code>` accordingly:
	...
	meson compile -C build kuzpkg-scripts-gmo kuzpkg-gmo libkuzpkg-gmo
	cp ./build/lib/libkuzpkg/po/<lang_code>/LC_MESSAGES/libkuzpkg.mo    /usr/share/locale/<lang_code>/LC_MESSAGES/libkuzpkg.mo
	cp ./build/scripts/po/<lang_code>/LC_MESSAGES/kuzpkg-scripts.mo /usr/share/locale/<lang_code>/LC_MESSAGES/kuzpkg-scripts.mo
	cp ./build/src/kuzpkg/po/<lang_code>/LC_MESSAGES/kuzpkg.mo      /usr/share/locale/<lang_code>/LC_MESSAGES/kuzpkg.mo


Translating Manpages
--------------------
There are currently no efforts underway to include translated manual pages in
the kuzpkg codebase. However, this is not to say translations are unwelcome. If
someone has experience with i18n man pages and how to best include them with our
source, please contact the kuzpkg-dev mailing list at
mailto:kuzpkg-dev@lists.archlinux.org[].
