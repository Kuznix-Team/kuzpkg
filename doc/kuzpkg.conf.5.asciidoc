...
Name
----
kuzpkg.conf - kuzpkg package manager configuration file


Synopsis
--------
{sysconfdir}/kuzpkg.conf


Description
-----------
Kuzpkg, using linkman:libkuzpkg[3], will attempt to read kuzpkg.conf each time it
is invoked. This configuration file is divided into sections or repositories.
Each section defines a package repository that kuzpkg can use when searching
...
*RootDir =* /path/to/root/dir::
	Set the default root directory for kuzpkg to install to. This option is
...
*DBPath =* /path/to/db/dir::
	...
	default is +{localstatedir}/lib/kuzpkg/+. Most users will not need to set
	this option. *NOTE*: if specified, this is an absolute path and the root
	path is not automatically prepended.

*CacheDir =* /path/to/cache/dir::
	Overrides the default location of the package cache directory. The
	default is +{localstatedir}/cache/kuzpkg/pkg/+. Multiple cache directories can be
...
*HookDir =* /path/to/hook/dir::
	Add directories to search for klpm hooks in addition to the system hook
	directory (+{datarootdir}/libkuzpkg/hooks/+).  The default is
	+{sysconfdir}/kuzpkg.d/hooks+.  Multiple directories can be specified with
	...
	automatically prepended.  For more information on the klpm hooks, see
	linkman:klpm-hooks[5].

*GPGDir =* /path/to/gpg/dir::
	Overrides the default location of the directory containing configuration
	files for GnuPG. The default is +{sysconfdir}/kuzpkg.d/gnupg/+.
...
*LogFile =* /path/to/log/file::
	Overrides the default location of the kuzpkg log file. The default
	is +{localstatedir}/log/kuzpkg.log+. This is an absolute path and the root directory
	is not prepended.

*HoldPkg =* package ...::
	If a user tries to '\--remove' a package that's listed in `HoldPkg`,
	kuzpkg will ask for confirmation before proceeding. Shell-style glob
	patterns are allowed.

*IgnorePkg =* package ...::
	Instructs kuzpkg to ignore any upgrades for this package when performing
	a '\--sysupgrade'. Shell-style glob patterns are allowed.

*IgnoreGroup =* group ...::
	Instructs kuzpkg to ignore any upgrades for all packages in this
...
*Architecture =* auto &| i686 &| x86_64 | ...::
	If set, kuzpkg will only allow installation of packages with the given
...
*Color*::
	Automatically enable colors only when kuzpkg's output is on a tty.
...
*DownloadUser =* username::
	...
	option is not set then the downloads are done as the user running kuzpkg.
...
Include = /etc/kuzpkg.d/mirrorlist
--------
...
+
During parsing, kuzpkg will define the `$repo` variable to the name of the
current section. This is often utilized in files specified using the 'Include'
directive so all repositories can use the same mirrorfile. kuzpkg also defines
...
directory with these packages so kuzpkg can find it when run with '\--refresh'.

	repo-add /home/pkgs/custom.db.tar.gz /home/pkgs/*.kuzpkg.tar.zst
...
in the configuration file as shown in the config example above. Kuzpkg will now
use your package repository. If you add new packages to the repository,
remember to re-generate the database and use kuzpkg's '\--refresh' option.
...
See Also
--------
linkman:kuzpkg[8], linkman:libkuzpkg[3]
