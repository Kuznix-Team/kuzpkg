...
klpm-hooks - klpm hook file format
...
libkuzpkg provides the ability to specify hooks to run before or after
...
+{datarootdir}/libkuzpkg/hooks+, and additional custom directories specified in
linkman:kuzpkg.conf[5] (the default is +{sysconfdir}/kuzpkg.d/hooks+).  The
