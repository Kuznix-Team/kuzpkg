...
customizations, generate meta-info, and package the whole thing up for kuzpkg
to use.
...
*-s, \--syncdeps*::
	Install missing dependencies using kuzpkg. When build-time or run-time
	dependencies are not found, kuzpkg will try to resolve them. If
...
*\--noconfirm*::
	(Passed to kuzpkg) Prevent kuzpkg from waiting for user input before
	proceeding with operations.

*\--needed*::
	(Passed to kuzpkg) Tell kuzpkg not to reinstall a target if it is already
	up-to-date. (used with '-i' / '\--install').

*\--asdeps*::
	(Passed to kuzpkg) Install packages as non-explicitly installed (used
	with '-i' / '\--install').

*\--noprogressbar*::
	(Passed to kuzpkg) Prevent kuzpkg from displaying a progress bar;
...
**KUZPKG**::
	...
	install and remove packages. Kuzpkg's '-Qq', '-Rns', '-S', '-T', and '-U'
	operations must be supported by this command. If the variable is not
	set or empty, makepkg will fall back to `kuzpkg'.
...
**PKGEXT=**".kuzpkg.tar.zst", **SRCEXT=**".src.tar.gz"::
...
See Also
--------
linkman:makepkg.conf[5], linkman:PKGBUILD[5], linkman:kuzpkg[8]
