...
*epoch*::
	...
	version comparison logic. See linkman:kuzpkg[8] for more information on
	version comparisons.
...
*backup (array)*::
	...
	`"Handling Config Files"` in linkman:kuzpkg[8] for more information.
...
*optdepends (array)*::
	...
	and are not utilized by kuzpkg during dependency resolution. Packages in
...
+
Sysupgrade is currently the only kuzpkg operation that utilizes this field.
...
*xdata (array)*::
	...
	This data is neither used by kuzpkg nor by makepkg;
...
--------------------------------
Kuzpkg has the ability to store and execute a package-specific script when it
...
See Also
--------
linkman:makepkg[8], linkman:kuzpkg[8], linkman:makepkg.conf[5]
