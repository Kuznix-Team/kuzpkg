...
Synopsis
--------
{sysconfdir}/makepkg.conf, $XDG_CONFIG_HOME/kuzpkg/makepkg.conf, ~/.makepkg.conf
...
$XDG_CONFIG_HOME/kuzpkg/makepkg.conf or ~/.makepkg.conf, with the former
taking priority.
...
**KUZPKG_AUTH=()**::
	Specify a command prefix for running kuzpkg as root. If unset, makepkg will
...
See Also
--------
linkman:makepkg[8], linkman:kuzpkg[8], linkman:PKGBUILD[5]
