...
**BUILDENV=(**!distcc !color !ccache check !sign**)**::
	...
	*sccache*;;
		Use sccache to cache compilation by default. This allows for faster
		compiles if you are continuously recompiling the same packages. It can
		be disabled for individual packages by placing `!sccache` in the
		PKGBUILD options array. Cannot be used alongside `distcc` and `ccache`.
...
**OPTIONS=(**!strip docs libtool staticlibs emptydirs ...**)**::
	...
	*autodepends*;;
		Enable the automatic addition of libraries to the depends array. Search
		library directories are controlled by the LIB_DIRS variable defined
		below.

	*autoprovides*;;
		Enable the automatic addition of libraries to the provides array. Search
		library directories are controlled by the LIB_DIRS variable defined
		below.
...
**LIB_DIRS=(**lib:usr/lib ...**)**::
	If `autodepends` or `autoprovides` is specified in the `OPTIONS` array, this
	variable will instruct makepkg where to look to find libraries to add to the
	`depends` and `provides` arrays. The format is "prefix:path", where provides will
	be added for libraries found in "path" with the specified prefix added.
