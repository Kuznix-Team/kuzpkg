...
**FRESHSOURCE=**"n"::
	If this value is set to "y", makepkg will download source files even if
	they already exist. This does not alter the normal update behavior for
	version control system sources. This can be overridden with makepkg's
	`\--freshsource` option.
