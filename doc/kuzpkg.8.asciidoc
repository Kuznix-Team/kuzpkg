...
Name
----
kuzpkg - package manager utility


Synopsis
--------
'kuzpkg' <operation> [options] [targets]


Description
-----------
Kuzpkg is a package management utility that tracks installed packages on a Linux
...
automatically upgrade packages. Kuzpkg packages are a zipped tar format.

Since version 3.0.0, kuzpkg has been the front-end to linkman:libkuzpkg[3], the
...
Invoking kuzpkg involves specifying an operation with any potential options and
...
*-D, \--database*::
	...
	certain attributes of the installed packages in kuzpkg's database. It
...
*-S, \--sync*::
	...
	example, `kuzpkg -S qt` will download and install qt and all the
	...
	package to install: `kuzpkg -S testing/qt`. You can also specify version
	requirements: `kuzpkg -S "bash>=3.2"`. Quotes are needed, otherwise the
	shell interprets ">" as redirection to a file.
+
In addition to packages, groups can be specified as well. For example, if
gnome is a defined package group, then `kuzpkg -S gnome` will provide a
...
+
Packages that provide other packages are also handled. For example, `kuzpkg -S
...
+
You can also use `kuzpkg -Su` to upgrade all packages that are out-of-date. See
<<SO,Sync Options>> below. When upgrading, kuzpkg performs version comparison
...
*-T, \--deptest*::
	...
	This operation accepts no other options. Example usage: `kuzpkg -T qt
	"bash>=3.2"`.
...
*-U, \--upgrade*::
	...
	on how kuzpkg takes care of configuration files.
...
*-b, \--dbpath* <path>::
	...
	+{localstatedir}/lib/kuzpkg+). This should not be used unless you know what
	you are doing.
...
*-r, \--root* <path>::
	...
	*NOTE*: If database path or log file are not specified on either the
	command line or in linkman:kuzpkg.conf[5], their default location will
...
*\--cachedir* <dir>::
	...
	+{localstatedir}/cache/kuzpkg/pkg+). Multiple cache directories can be
	specified, and they are tried in the order they are passed to kuzpkg.
	*NOTE*: This is an absolute path, and the root path is not automatically
	prepended. If DownloadUser is set in linkman:kuzpkg.conf[5], then the
...
*\--gpgdir* <dir>::
	...
	(the default is +{sysconfdir}/kuzpkg.d/gnupg+). This directory should contain
...
*\--hookdir* <dir>::
	...
	+{sysconfdir}/kuzpkg.d/hooks+). Multiple hook directories can be specified
...
*\--noconfirm*::
	...
	this unless you want to run kuzpkg from a script.
...
*-d, \--nodeps*::
	...
	kuzpkg will always check a package's dependency fields to ensure that all
...
*\--noprogressbar*::
	...
	for scripts that call kuzpkg and capture the output.
...
*\--ignore* <package>::
	Directs kuzpkg to ignore upgrades of package even if there is one
	available. Multiple packages can be specified by separating them
	with a comma.

*\--ignoregroup* <group>::
	Directs kuzpkg to ignore upgrades of all packages in 'group', even if
...
*-q, \--quiet*::
	...
	kuzpkg's output is processed in a script. Search will only show package
...
*-n, \--nosave*::
	Instructs kuzpkg to ignore file backup designations. Normally, when a
...
*-c, \--clean*::
	...
	currently unused sync databases to free up disk space. When kuzpkg
	...
	linkman:kuzpkg.conf[5]. Use one '\--clean' switch to only remove
...
*-u, \--sysupgrade*::
	...
	necessary.
+
Pass this option twice to enable package downgrades; in this case, kuzpkg will
...
*-y, \--refresh*::
	...
	server(s) defined in linkman:kuzpkg.conf[5]. This should typically be used
...
*-q, \--quiet*::
	...
	kuzpkg's output is processed in a script, however, you may want to use
...
----------------------------
Kuzpkg uses the same logic as 'rpm' to determine action against files that are
...
kuzpkg -U /home/user/ceofhack-0.6-1-x86_64.kuzpkg.tar.zst::
...
Configuration
-------------
See linkman:kuzpkg.conf[5] for more details on configuring kuzpkg using the
'kuzpkg.conf' file.


See Also
--------
linkman:klpm-hooks[5], linkman:libkuzpkg[3], linkman:makepkg[8],
linkman:kuzpkg.conf[5]
